	...
	return (
		...
		>
			...
			<span className="mt-auto flex w-full flex-col items-start gap-2">
				<span className="text-[16px] font-bold leading-6 text-[var(--color-text-import-title)]">{title}</span>
	...
	return (
		<div className="flex items-start justify-between gap-4 border-b border-[var(--color-border-agents-sheet)] p-(--size-import-dialog-padding)">
			<div className="min-w-0">
				<Title className="text-subtitle font-semibold text-[var(--color-text-agents-sheet-title)]">{title}</Title>
...
export function ProjectSettingsValueRow({
	...
	return (
		<ProjectSettingsRow icon={icon} label={label}>
			{href && ExternalLink ? (
				<ExternalLink href={href} className="settings-row-value text-settings-accent hover:underline" title={value}>
					{value}
				</ExternalLink>
			) : (
...
export function ProjectSettingsFormView({ children, className, onSubmit, ...props }: ProjectSettingsFormProps) {
	return (
		<form
			{...props}
