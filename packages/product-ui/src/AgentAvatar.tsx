...
export function AgentAvatar({ provider, className, decorative = false, logoSrc, logoSources }: AgentAvatarProps) {
	const identity = getAgentIdentity(provider);
	const logo = logoSrc ?? (identity.logoKey ? logoSources?.[identity.logoKey] : undefined);
	if (logo) {
		const imageAccessibility: Pick<ImgHTMLAttributes<HTMLImageElement>, "alt" | "aria-hidden" | "title"> = decorative
	...
	const fallbackAccessibility: Pick<
		HTMLAttributes<HTMLSpanElement>,
		"aria-hidden" | "aria-label" | "role" | "title"
	> = decorative ? { "aria-hidden": true } : { "aria-label": provider, role: "img", title: provider };
	return (
		<span
