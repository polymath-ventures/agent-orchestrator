...
import { TaskComposerView, type TaskComposerViewProps } from "./TaskComposerView";
...
function viewProps(overrides: Partial<TaskComposerViewProps> = {}): TaskComposerViewProps {
	return {
		...
		renderModelControl: (control) => (
			<input aria-label="Model" value={control.value} onChange={(event) => control.onModelChange(event.target.value)} />
		),
		...overrides,
	};
}
