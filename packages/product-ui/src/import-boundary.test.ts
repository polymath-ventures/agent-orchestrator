...
describe("product-ui import boundary", () => {
	it("keeps production sources inside the portable package boundary", () => {
		...
		for (const file of productionSourceFiles(sourceRoot)) {
			...
			if (/<a(?:\s|>)/.test(source)) {
				violations.push(
					`${relative(sourceRoot, file)} owns a direct anchor instead of using the external-link boundary`,
				);
			}
...
function productionSourceFiles(directory: string): string[] {
	return readdirSync(directory).flatMap((entry) => {
		...
		return /\.(ts|tsx)$/.test(entry) && !entry.endsWith(".test.ts") && !entry.endsWith(".test.tsx") ? [path] : [];
	});
}
