import { type ClipboardEvent, type DragEvent, type FormEvent, type ReactNode, useId, useRef, useState } from "react";
import { FileTextIcon as FileText, LoaderCircleIcon as Loader2, PaperclipIcon as Paperclip, XIcon as X } from "./icons";
		...
		>
			...
			<div className="composer-toolbar">
				<div className="composer-run-controls" role="group" aria-label={labels.runsWith}>
					<div className="composer-toolbar-slot">{renderAgentControl({ ...agent, id: agentId })}</div>
					<span className="composer-toolbar-divider" aria-hidden="true" />
					<div className="composer-toolbar-slot">{renderModelControl({ ...model, id: modelId })}</div>
				</div>
				<button
					type="button"
