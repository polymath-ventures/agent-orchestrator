...
export type SessionsBoardGridViewProps<TSession extends BoardSessionPresentation = BoardSessionPresentation> = {
			...
			<div className="flex h-12 shrink-0 items-center gap-2.5 px-4">
				<span
					...
					style={{
						...
						boxShadow: column.dotGlow ? `0 0 7px color-mix(in srgb, ${column.dot} 60%, transparent)` : undefined,
					}}
				/>
...
function splitLaneTone(tone: "idle" | "working" | "ready" | "merged", labels: BoardSplitLaneToneLabels): SplitLaneTone {
	const styles = {
		idle: {
				...
				{prs.length > 0 && (
					<div className="flex flex-wrap items-center gap-x-2 gap-y-1 font-mono text-2xs text-passive">
						{groupBoardPullRequests(prs).map((group) => (
							<BoardPullRequestGroup externalLink={externalLink} group={group} key={group.state} labels={labels.pr} />
						))}
					</div>
				)}
...
export function groupBoardPullRequests(prs: BoardPullRequestPresentation[]): BoardPullRequestGroupModel[] {
