...
function PRDiffMeta({ countNounLabel, pr }: { countNounLabel: CountNounLabel; pr: PRSummaryMetadata }) {
	...
	return (
		...
		>
			{visibleParts.map((part) => {
				...
				const overflowLabel =
					extra > 0 ? (part.overflowNoun ? `+${countNounLabel(extra, part.overflowNoun)}` : `+${extra}`) : undefined;
				return (
					<div className={cn("min-w-0", stacked ? "flex flex-col" : "inline-flex flex-wrap gap-x-1")} key={part.key}>
...
export function prBranchRange(pr: Pick<PRSummaryMetadata, "sourceBranch" | "targetBranch">): string | undefined {
...
export function hasDiffMetadata(pr: Pick<PRSummaryMetadata, "changedFiles" | "additions" | "deletions">): boolean {
