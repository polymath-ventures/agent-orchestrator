...
describe("portable leaf components", () => {
	it("renders an injected agent logo without owning app assets", () => {
		...
		expect(screen.getByRole("img", { name: "claude-code" })).toHaveAttribute("src", "/logos/claude.svg");
	});
	...
	it("renders precomputed PR presentation without controller dependencies", () => {
		...
		const { container } = render(<PRCardStatusSummary externalLink={ExternalLink} presentation={presentation} />);
	...
	it("computes overflow against the requested link limit", () => {
		...
		render(<PRSummaryParts externalLink={ExternalLink} interactiveLinks={false} maxLinks={2} parts={parts} />);
