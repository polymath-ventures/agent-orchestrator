...
export function AgentLogo({ harness, size = 24 }: { harness?: string | null; size?: number }) {
	...
	const chip = polarity === "needs-dark" ? DARK_CHIP : polarity === "needs-light" ? LIGHT_CHIP : "transparent";
	...
	return (
		<View
			style={[styles.box, { width: size, height: size, borderRadius: Math.round(size * 0.28), backgroundColor: chip }]}
		>
			{source ? (
				<Image
					source={source}
