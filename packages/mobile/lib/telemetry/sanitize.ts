...
function keepValue(rule: PropRule, value: unknown): { ok: boolean; value?: unknown } {
	if ("oneOf" in rule) {
		return typeof value === "string" && rule.oneOf.includes(value) ? { ok: true, value } : { ok: false };
	}
	if ("flag" in rule) {
...
export function sanitizeMobileProperties(event: string, properties?: Record<string, unknown>): Record<string, unknown> {
