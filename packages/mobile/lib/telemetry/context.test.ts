...
describe("buildMobileContext", () => {
	...
	it("distinguishes dev, simulator, and device builds", () => {
		expect(
			buildMobileContext({ platformOS: "android", isPhysicalDevice: true, dev: true, appVersion: "x" }).build_mode,
		).toBe("dev");
		expect(
			buildMobileContext({ platformOS: "android", isPhysicalDevice: false, dev: false, appVersion: "x" }).build_mode,
		).toBe("simulator");
		expect(
			buildMobileContext({ platformOS: "android", isPhysicalDevice: true, dev: false, appVersion: "x" }).build_mode,
		).toBe("device");
	});
