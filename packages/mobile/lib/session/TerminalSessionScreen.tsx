...
export default function TerminalScreen() {
	...
	useLayoutEffect(() => {
			...
			headerRight: () => (
				<View style={styles.headerActions}>
					...
					<Pressable
						...
						style={({ pressed }) => [styles.headerBrowserBtn, pressed && { opacity: 0.6 }]}
					>
						<Feather name="globe" size={19} color={browserOpen ? t.blue : hasPreview ? t.green : t.textSecondary} />
		...
		interfaceSwitch.status?.supported,
	...
	const confirmKill = useCallback(() => {
		...
		haptics.warning();
		Alert.alert(
			shellOnly ? "Close shell?" : "Kill session?",
			shellOnly ? "This stops the worktree shell." : `This stops ${id}.`,
			[
			...
			(interfaceSwitch.transition?.phase === "failed" || interfaceSwitch.transition?.phase === "recovery_required") ? (
				...
				>
					<Text style={styles.bannerText}>
						{interfaceSwitch.transition.errorDetail || "The interface switch failed; Terminal UI remains available."}
					</Text>
				</Pressable>
			) : null}
							...
							{mobileInterfaceTransitionIsCancellable(interfaceSwitch.transition) ? (
								...
								>
									<Text style={styles.interfaceCancelText}>
										{interfaceSwitch.cancelling ? "Cancelling…" : "Cancel switch"}
									</Text>
								</Pressable>
							) : null}
