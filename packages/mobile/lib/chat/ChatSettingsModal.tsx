...
function GroupedChoices({
	...
	disabled?: boolean;
	onOption(id: string, value: { value: string }): void;
}) {
	const styles = useThemedStyles(makeStyles);
	...
	return (
		<>
			{[...groups.entries()].map(([group, choices]) => (
				<View key={group || "default"}>
					...
					{choices.map((choice) => (
						<Choice
							...
							disabled={disabled}
							onPress={() => onOption(option.id, { value: choice.value })}
						/>
					))}
				</View>
			))}
	...
	return (
		<View style={styles.section}>
			<View style={styles.sectionTitle}>
				<Feather name={icon} size={14} color={t.textTertiary} />
...
function Choice({
	...
	return (
		<Pressable
			...
			style={({ pressed }) => [styles.choice, pressed && { backgroundColor: t.bgSubtle }]}
		>
			<View style={{ flex: 1 }}>
				<Text style={[styles.choiceLabel, selected && { color: t.blue }]}>{label}</Text>
				{hint ? <Text style={styles.choiceHint}>{hint}</Text> : null}
			</View>
			{selected ? <Feather name="check" size={16} color={t.blue} /> : null}
		</Pressable>
	);
}
...
function configOptionIcon(option: ChatConfigOption): keyof typeof Feather.glyphMap {
	if (option.id === "fast") return "zap";
	if (option.id === "agent") return "user";
