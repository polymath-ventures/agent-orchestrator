...
function MarkdownImage({ alt, url }: { alt: string; url: string }) {
	...
	return (
		<View style={styles.imageCard}>
			<Image
				accessibilityLabel={alt || "Conversation image"}
		...
		<View style={styles.codeCard}>
			<View style={styles.codeHeader}>
				...
				<Pressable
					...
					accessibilityLabel={wrap ? "Disable code wrapping" : "Wrap code"}
					accessibilityState={{ selected: wrap }}
					onPress={() => {
						preferredCodeWrap = !wrap;
						setWrap(!wrap);
					}}
					style={styles.copyButton}
				>
					<Feather name="corner-down-left" size={13} color={wrap ? t.blue : t.textTertiary} />
					<Text style={[styles.copyLabel, wrap && { color: t.blue }]}>Wrap</Text>
				</Pressable>
				<Pressable
					accessibilityRole="button"
			...
			<ScrollView
				...
				contentContainerStyle={wrap ? styles.codeWrap : undefined}
			>
				<HighlightedCodeText
					code={text}
					language={language}
					streaming={streaming}
					style={[styles.codeText, wrap && styles.codeTextWrap]}
				/>
			</ScrollView>
		</View>
	);
...
function inline(text: string, styles: ReturnType<typeof makeStyles>): ReactNode[] {
	...
	while ((match = pattern.exec(text))) {
		...
		if ((match[2] && match[3]) || match[4] || match[11]) {
			...
			nodes.push(
				<Text key={`${match.index}-code`} style={styles.inlineCode}>
					{match[5]}
				</Text>,
			);
		} else if (match[6] || match[7]) {
			nodes.push(
				<Text key={`${match.index}-strong`} style={styles.strong}>
					{match[6] ?? match[7]}
				</Text>,
			);
		} else if (match[8]) {
			nodes.push(
				<Text key={`${match.index}-strike`} style={styles.strike}>
					{match[8]}
				</Text>,
			);
		} else {
			nodes.push(
				<Text key={`${match.index}-em`} style={styles.em}>
					{match[9] ?? match[10]}
				</Text>,
			);
		}
