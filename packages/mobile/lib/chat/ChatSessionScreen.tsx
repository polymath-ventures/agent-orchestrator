...
export function ChatSessionScreen({ session }: { session: MobileChatSession }) {
	...
	const openShell = useCallback(async () => {
		...
		try {
			...
			setMenuOpen(false);
			router.push({
				pathname: "/shell/[handleId]",
				params: { handleId: shell.handleId, projectId: session.projectId, sessionId: session.id, title: shell.title },
			});
		} catch (cause) {
	...
	const requestInterfaceSwitch = useCallback(() => {
		if (!interfaceSwitch.status?.supported) {
			Alert.alert(
				"Terminal UI unavailable",
				interfaceSwitch.status?.reason ||
					interfaceSwitch.error ||
					"This agent cannot resume the same native conversation in Terminal UI.",
			);
			return;
		}
		if (!turnActive) {
	...
	if (conversation.loading && !conversation.snapshot)
		return <Centered icon="message-square" title="Loading conversation…" spinning />;
	if (conversation.unavailable)
		return (
			<Unavailable
				message={conversation.unavailable.message}
				onShell={() => void openShell()}
	...
	if (!conversation.snapshot)
		return (
			<Centered
				...
				message={conversation.error || "The daemon did not return a conversation."}
				action="Retry"
	...
	const activeTurn =
		snapshot.turns.find((turn) => turn.state === "running") ?? snapshot.turns.find((turn) => turn.state === "queued");
	...
	const mcpReloadSupported =
		can(snapshot, "mcp_reload") && !conversationActionUnsupported("mcp", conversation.actionCodes.mcp);
		...
		>
			...
			{interfaceTransitionActive ? (
			  ...
			  interfaceSwitch.transition?.phase === "recovery_required" ? (
				<InlineBanner
					...
					icon="alert-triangle"
					text={
						interfaceSwitch.transition.errorDetail ||
			...
			{conversation.error ? (
				<InlineBanner
					...
					action="Retry"
					onPress={() => void conversation.refresh()}
				/>
			) : null}
			{quota ? (
				<InlineBanner
					tone={quota.severity === "critical" ? "danger" : "warning"}
					icon="alert-triangle"
					text={`${quota.percent}% of the${quota.planLabel ? ` ${quota.planLabel}` : ""} account quota is used${resetLabel(quota.resetsInSeconds) ? `; resets in ${resetLabel(quota.resetsInSeconds)}` : ""}. ${quota.severity === "critical" ? "Turns may start failing for reasons unrelated to your request." : "Turns will stop when the limit is reached."}`}
					action="Details"
					onPress={() => setMenuOpen(true)}
				/>
			) : null}
			{conversation.actionError ? (
				<InlineBanner tone="danger" icon="alert-circle" text={conversation.actionError} />
			) : null}
			{rolledBack ? (
				<InlineBanner
					tone="muted"
					icon="rotate-ccw"
					text={`${rolledBack} ${rolledBack === 1 ? "turn was" : "turns were"} rolled back. The agent no longer remembers ${rolledBack === 1 ? "it" : "them"}.`}
				/>
			) : null}
			{conversation.pendingSends.map((pendingSend) =>
				pendingSend.state === "failed" ? (
					<InlineBanner
						key={pendingSend.id}
						tone="danger"
						icon="send"
						text={`Message not sent: ${pendingSend.error || "Delivery failed"}`}
						action="Retry"
						secondary="Discard"
						onPress={() => void conversation.retrySend(pendingSend.id).catch(() => {})}
						onSecondary={() => conversation.discardSend(pendingSend.id)}
					/>
				) : null,
			)}
			<ChatTimeline
			...
			{activeTurn ? (
				<LiveTurnBar
					...
					stopping={conversation.pendingActions.includes("interrupt")}
			...
			<ConversationMenu
				...
				onPreview={() => {
					...
					router.push({
						...
						params: {
							id: session.id,
							title,
							previewUrl: "previewUrl" in session ? (session.previewUrl ?? undefined) : undefined,
						},
					});
				}}
				onPullRequests={() => {
				...
				onCompact={() => {
					setMenuOpen(false);
					void conversation.compact().catch(() => {});
				}}
				onReload={() => {
					setMenuOpen(false);
					void conversation.reloadMcp().catch(() => {});
				}}
				onRename={(next) => {
					setMenuOpen(false);
...
function ConversationBanners({
	...
	snapshot: NonNullable<ReturnType<typeof useMobileConversation>["snapshot"]>;
	brokenServers: ReturnType<typeof brokenMcpServers>;
	resuming: boolean;
	terminated: boolean;
	...
	return (
		<>
			{snapshot.account?.reauthRequiredAt ? (
				<InlineBanner
					tone="danger"
					icon="key"
					text={`${snapshot.account.reauthReason || "The provider rejected this session's credentials."} ${signIn ? `Run “${signIn}” on the AO host, then try again.` : "Sign in with the agent's CLI on the AO host, then try again."} AO holds no credentials of its own. The worktree is untouched.`}
					action="Open shell"
					onPress={onOpenShell}
				/>
			) : null}
			{snapshot.controller.state === "stopped" ? (
				<InlineBanner
					...
					text={
						terminated
							? "This AO session is terminated. Its conversation and worktree are preserved."
							: snapshot.controller.error || "The agent controller is stopped."
					}
					action={terminated ? (resuming ? "Restoring…" : "Restore session") : resuming ? "Resuming…" : "Resume agent"}
					secondary="Shell"
					onPress={resuming ? undefined : onResume}
					onSecondary={onOpenShell}
				/>
			) : null}
			{snapshot.controller.state === "recovering" || snapshot.controller.state === "connecting" ? (
				<InlineBanner
					tone="warning"
					icon="loader"
					text={
						snapshot.controller.state === "recovering" ? "Reconnecting to the agent…" : "Starting the agent controller…"
					}
				/>
			) : null}
			{thread?.status === "system_error" ? (
				<InlineBanner
					tone="danger"
					icon="alert-triangle"
					text={`The provider reports an internal fault in this thread; AO's connection may still be healthy. The conversation and worktree are kept.${thread.waitingOn?.length ? ` Waiting on: ${thread.waitingOn.join(", ")}.` : ""}`}
				/>
			) : thread?.status === "closed" ? (
				<InlineBanner
					tone="warning"
					icon="alert-triangle"
					text={`The provider closed this thread. AO kept its history, but the agent no longer holds it.${thread.waitingOn?.length ? ` Waiting on: ${thread.waitingOn.join(", ")}.` : ""}`}
				/>
			) : null}
			{brokenServers.length ? (
				<InlineBanner
					tone="warning"
					icon="tool"
					text={`${brokenServers.map(mcpServerFailureLabel).join(", ")} did not start. The agent has none of their tools and will not say so—it works around them silently.${mcpError ? ` Reload failed: ${mcpError}` : ""}`}
					action={mcpReloadSupported && !turnInFlight ? (mcpReloading ? "Reloading…" : "Reload") : undefined}
					onPress={mcpReloading ? undefined : onReload}
				/>
			) : null}
		</>
	);
...
function LiveTurnBar({
	...
	const blocked = snapshot.items.some(
		(item) =>
			item.kind === "activity" &&
			(item.activityKind === "approval" || item.activityKind === "user_input") &&
			item.status === "pending",
	...
	return (
		<View style={styles.live}>
			<ActivityIndicator size="small" color={blocked ? t.amber : t.orange} />
			...
			>
				...
				<Text style={styles.stopTurnText}>{stopping ? "Stopping…" : stopLabel}</Text>
			</Pressable>
		</View>
	);
}
			...
			<View style={styles.menu}>
				{renaming ? (
					<View style={styles.rename}>
						<Text style={styles.menuHeading}>Rename conversation</Text>
						<TextInput
							autoFocus
							value={title}
					...
					<ScrollView>
						...
						<MenuRow
							icon="repeat"
							label={interfaceSwitching ? "Switching interface…" : "Open Terminal UI"}
							hint={
								interfaceSupported
									? "Resume this native conversation in the agent's terminal"
									: interfaceReason || "This agent has not declared a compatible handoff"
							}
							disabled={!interfaceSupported || interfaceSwitching}
							onPress={onSwitchInterface}
						/>
						<MenuRow icon="list" label="Conversation map" hint="Jump to any request and response" onPress={onMap} />
						<MenuRow
							icon="terminal"
							label={openingShell ? "Opening shell…" : "Open worktree shell"}
						...
						/>
						{can(snapshot, "rename") ? (
							<MenuRow icon="edit-2" label="Rename" onPress={() => setRenaming(true)} />
						) : null}
						{compactSupported ? (
							<MenuRow
								icon="archive"
								label={compacting ? "Compacting history…" : "Compact history"}
								hint={
									turnInFlight
										? "Available after the current turn finishes"
										: snapshot.compactedAt
											? `Last compacted ${new Date(snapshot.compactedAt).toLocaleString()}`
											: "Summarize older context without changing files"
								}
								disabled={turnInFlight || compacting}
								onPress={onCompact}
							/>
						) : null}
						{mcpReloadSupported ? (
							<MenuRow
								icon="refresh-cw"
								label={mcpReloading ? "Reloading MCP servers…" : "Reload MCP servers"}
								hint={turnInFlight ? "Available after the current turn finishes" : undefined}
								disabled={turnInFlight || mcpReloading}
								onPress={onReload}
							/>
						) : null}
						{snapshot.usage ? (
							<View style={styles.rateBox}>
								<Text style={styles.rateTitle}>Context and usage</Text>
								<Text style={styles.rateText}>
									{formatTokens(snapshot.usage.contextUsed)} / {formatTokens(snapshot.usage.contextWindow)} context ·{" "}
									{formatTokens(snapshot.usage.inputTokens)} in · {formatTokens(snapshot.usage.outputTokens)} out
									{snapshot.usage.cachedTokens ? ` · ${formatTokens(snapshot.usage.cachedTokens)} cached` : ""}
									{snapshot.usage.cost != null
										? ` · ${snapshot.usage.currency || "$"}${snapshot.usage.cost.toFixed(4)}`
										: ""}
								</Text>
							</View>
						) : null}
						{snapshot.rateLimits ? (
							<View style={styles.rateBox}>
								<Text style={styles.rateTitle}>{snapshot.rateLimits.planLabel || "Rate limits"}</Text>
								<Text style={styles.rateText}>
									Primary: {Math.round(snapshot.rateLimits.primaryUsedPercent)}% used
									{formatReset(snapshot.rateLimits.primaryResetsInSeconds)}
									{snapshot.rateLimits.secondaryUsedPercent >= 0
										? ` · Secondary: ${Math.round(snapshot.rateLimits.secondaryUsedPercent)}%${formatReset(snapshot.rateLimits.secondaryResetsInSeconds)}`
										: ""}
								</Text>
							</View>
						) : null}
...
function MenuRow({
	...
	return (
		...
		>
			<Feather name={icon} size={16} color={t.textTertiary} />
			<View style={{ flex: 1 }}>
...
function Unavailable({ message, onShell, openingShell }: { message: string; onShell(): void; openingShell: boolean }) {
	return (
		<Centered
			icon="alert-triangle"
			title="Conversation unavailable"
			message={`${message}\n\nThe worktree is untouched. You can still open a plain shell in it.`}
			action={openingShell ? "Opening…" : "Open worktree shell"}
			onAction={onShell}
		/>
	);
}
function Centered({
	...
	return (
		<View style={styles.center}>
			{spinning ? <ActivityIndicator color={t.blue} /> : <Feather name={icon} size={22} color={t.amber} />}
...
function sessionTitle(session: MobileChatSession): string {
	return "displayName" in session
		? session.displayName || session.issueTitle || session.issueLabel || session.id
...
function interfacePhaseLabel(phase?: string): string {
	switch (phase) {
		...
		case "source_stopping":
			return "stopping Chat controller";
		case "source_stopped":
			return "Chat controller stopped";
		case "target_starting":
			return "resuming terminal controller";
		case "activating":
			return "opening Terminal UI";
		default:
			return "preparing native handoff";
	}
}
...
function formatReset(seconds?: number): string {
	...
	if (seconds < 60) return ` · resets in ${Math.ceil(seconds)}s`;
