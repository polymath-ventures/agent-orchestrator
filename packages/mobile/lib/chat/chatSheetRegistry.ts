...
export type ChatSheetEntry =
	| {
			...
			onRefresh(): Promise<{ models: ChatModel[]; configOptions: ChatConfigOption[] }>;
	  }
...
export function readChatSheet(key?: string): ChatSheetEntry | undefined {
	return key ? entries.get(key) : undefined;
}
export function releaseChatSheet(key?: string): void {
	if (key) entries.delete(key);
}
export function resetChatSheets(): void {
	entries.clear();
	sequence = 0;
}
...
export function chatSheetRoute(entry: ChatSheetEntry) {
	...
	const pathname =
		entry.kind === "turn-settings"
			? "/sheets/chat-settings"
			: entry.kind === "conversation-map"
				? "/sheets/conversation-map"
				: "/sheets/composer-picker";
