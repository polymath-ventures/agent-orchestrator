...
export function conversationActionError(error: unknown): string {
	...
	switch (code) {
		...
		case "CHAT_TURN_NOT_STEERABLE":
			return `${error instanceof Error ? error.message : "This turn cannot be steered right now."} Try again when it finishes, or queue a new message.`;
		...
		case "CHAT_AUTH_REQUIRED":
			return "Sign in with the agent CLI on the AO host, then resume this session.";
		case "CHAT_DRIVER_UNAVAILABLE":
			return "The agent CLI is unavailable on the AO host. Install it or open the worktree shell.";
		case "CHAT_DRIVER_INCOMPATIBLE":
			return "The installed agent CLI is not compatible with AO Chat. Update it, then resume this session.";
		case "CHAT_RESUME_FAILED":
			return "AO could not resume this agent. The conversation and worktree are preserved.";
		case "CHAT_PROVIDER_REFUSED":
			return error instanceof Error ? error.message : "The provider refused this action.";
		default:
