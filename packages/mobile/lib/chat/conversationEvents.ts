...
import { createConversationEventRegistry, createCursorPersister, type ConversationEvent } from "./sse";
...
export function useConversationEventTransport(cfg: ServerConfig | null): void {
	useEffect(() => {
		...
		const cursorPersister = createCursorPersister((cursor) => AsyncStorage.setItem(cursorKey, String(cursor)));
		const run = async () => {
