...
describe("turn option catalog", () => {
	it("loads the capability-specific catalog without swallowing failures", async () => {
		...
		expect(await loadTurnOptionCatalog({ hasProviderConfig: false, loadModels, loadConfigOptions })).toEqual({
			models: [{ id: "gpt" }],
			configOptions: [],
		});
		...
		await expect(loadTurnOptionCatalog({ hasProviderConfig: true, loadModels, loadConfigOptions })).rejects.toThrow(
			"catalog unavailable",
		);
	});
});
