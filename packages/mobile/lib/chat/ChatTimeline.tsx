...
function ConversationTurnGroup({
	...
	return (
		<View>
			...
			{group.turn ? (
				<TurnSummary turn={group.turn} onRollback={canRollbackTurn(snapshot, group.turn) ? onRollback : undefined} />
			) : null}
		</View>
	);
}
...
const TimelineItem = memo(function TimelineItem({
	...
	if (item.activityKind === "system" && item.detail?.event === "steer") {
		return (
			<View style={styles.userRow}>
				<View style={[styles.userBubble, styles.steerBubble]}>
					<Text style={styles.steerLabel}>STEERED</Text>
					<Text selectable style={styles.userText}>
						{item.detail.text || item.summary}
					</Text>
				</View>
			</View>
		);
	}
	if (item.detail?.event === "model.rerouted")
		return (
			<SystemSignal
				icon="shuffle"
				title={`Answered by ${item.detail.toModel || "another model"}`}
				detail={
					item.detail.fromModel
						? `Instead of ${item.detail.fromModel}${item.detail.reason ? ` · ${item.detail.reason}` : ""}`
						: item.detail.reason
				}
			/>
		);
	if (item.detail?.event === "auth.reauth_required")
		return (
			<SystemSignal icon="key" danger title="The provider asked you to sign in again" detail={item.detail.reason} />
		);
...
function SystemSignal({
	...
	const styles = useThemedStyles(makeStyles);
	return (
		<View style={[styles.systemSignal, danger && { borderColor: t.red }]}>
			<Feather name={icon} size={14} color={danger ? t.red : t.textTertiary} />
			<View style={{ flex: 1 }}>
				<Text style={[styles.systemTitle, danger && { color: t.red }]}>{title}</Text>
				{detail ? <Text style={styles.systemDetail}>{String(detail)}</Text> : null}
			</View>
		</View>
	);
}
	...
	return (
		<View style={styles.originMessage}>
			<View style={styles.originHeader}>
				<Feather name="radio" size={11} color={t.textTertiary} />
				<Text style={styles.originLabel}>
					{message.senderLabel || (message.origin === "automation" ? "Automation" : "AO")}
				</Text>
			</View>
			{long && expanded ? (
				<ChatMarkdown text={message.text} />
			) : (
				<Text selectable numberOfLines={long ? 5 : undefined} style={styles.originText}>
					{message.text}
				</Text>
			)}
			{long ? (
				...
				>
					...
					<Text style={styles.originMoreText}>{expanded ? "Hide report" : "Show full report"}</Text>
				</Pressable>
			) : null}
		</View>
	);
...
function GenericActivityRow({ activity }: { activity: ConversationActivity }) {
	...
	const expandable = Boolean(
		output || detail.cwd || detail.arguments || detail.files || detail.reason || detail.text || detail.terminalInput,
	);
		...
		<View style={styles.activityWrap}>
			...
			>
				...
				<Text
					...
					style={[styles.activitySummary, activity.status === "failed" && { color: t.red }]}
				>
			...
			{open ? (
				<View style={styles.activityDetail}>
					...
					{detail.outputTruncated || detail.patchOutputTruncated ? (
						<Text style={[styles.partial, { color: t.amber }]}>
							This output is longer than AO stores, so it stops early. Open the worktree shell for the full run.
						</Text>
					) : detail.outputMayBePartial ? (
						<Text style={styles.partial}>
							{detail.outputSource === "stream"
								? "Streamed live; the provider may have omitted the beginning."
								: "The provider's rolled-up output may omit the beginning."}{" "}
...
function TerminalInput({ text, truncated }: { text: string; truncated?: boolean }) {
	...
	return (
		<View style={styles.terminalInput}>
			<View style={styles.terminalInputTitle}>
				<Feather name="corner-down-right" size={11} color={t.textTertiary} />
				<Text style={styles.detailLabel}>AGENT TYPED</Text>
			</View>
			<CodeOutput value={caretNotation(text)} />
			{truncated ? (
				<Text style={styles.partial}>AO stopped recording keystrokes at its cap; more were sent.</Text>
			) : null}
		</View>
	);
}
...
function McpToolRow({ activity }: { activity: ConversationActivity }) {
	...
	const body =
		detail.arguments !== undefined || detail.result !== undefined || Boolean(detail.error || detail.progress);
	return (
		<View style={styles.activityWrap}>
			<Pressable
				disabled={!body}
			...
			>
				...
				<Text style={[styles.server, failed && { color: t.red }]}>
					{(detail.server ?? detail.namespace) ? `${detail.server ?? detail.namespace}/` : ""}
				</Text>
				<Text numberOfLines={1} style={[styles.activitySummary, failed && { color: t.red }]}>
					{detail.toolName || activity.summary}
				</Text>
				{detail.progress ? (
					<Text numberOfLines={1} style={styles.activityProgress}>
						{lastLine(detail.progress)}
					</Text>
				) : null}
				{activity.status === "running" ? (
					<ActivityIndicator size="small" color={t.purple} />
				) : activity.status === "cancelled" ? (
					<Text style={styles.activityStopped}>stopped</Text>
				) : body ? (
					<Feather name={open ? "chevron-up" : "chevron-right"} size={13} color={t.textFaint} />
				) : null}
			</Pressable>
			{open && body ? (
				<View style={styles.activityDetail}>
					{detail.error ? <Text style={[styles.detailCopy, { color: t.red }]}>{detail.error}</Text> : null}
					{detail.arguments !== undefined ? <JsonPayload label="Arguments" value={detail.arguments} /> : null}
					{detail.result !== undefined ? <JsonPayload label="Result" value={detail.result} /> : null}
					{detail.progress ? (
						<View>
							<Text style={styles.detailLabel}>PROGRESS</Text>
							<CodeOutput value={detail.progress} />
							{detail.progressTruncated ? (
								<Text style={styles.partial}>Progress was longer than AO stores.</Text>
							) : null}
						</View>
					) : null}
				</View>
...
function JsonPayload({ label, value }: { label: string; value: unknown }) {
	...
	return (
		<View>
			...
			{note ? <Text style={styles.detailCopy}>{note}</Text> : <CodeOutput value={printable(value)} />}
		</View>
	);
}
...
function AutoReviewRow({ activity }: { activity: ConversationActivity }) {
	...
	const detail = activity.detail ?? {};
	const denied = String(detail.status ?? "")
		.toLowerCase()
		.includes("den");
	const paths = reviewPaths(detail.files);
	const body = Boolean(
		detail.rationale || detail.command || detail.cwd || detail.host || detail.decisionSource || paths.length,
	);
	return (
		<View style={styles.activityWrap}>
			<Pressable
			...
			>
				...
				<Text style={[styles.reviewDecision, denied && { color: t.red }]}>
				...
				{detail.riskLevel ? (
					<Text
						style={[styles.risk, ["high", "critical"].includes(detail.riskLevel.toLowerCase()) && { color: t.red }]}
					>
						{detail.riskLevel}
					</Text>
				) : null}
				{body ? <Feather name={open ? "chevron-up" : "chevron-right"} size={13} color={t.textFaint} /> : null}
			</Pressable>
			{open && body ? (
				<View style={styles.activityDetail}>
					...
					{detail.rationale ? <Text style={styles.reviewRationale}>{detail.rationale}</Text> : null}
					{detail.command ? <LabelValue label="cmd" value={detail.command} /> : null}
					{detail.cwd ? <LabelValue label="cwd" value={detail.cwd} /> : null}
					{detail.host ? <LabelValue label="host" value={detail.host} /> : null}
					{paths.length ? <LabelValue label="files" value={paths.join(", ")} /> : null}
					{detail.decisionSource ? <LabelValue label="by" value={detail.decisionSource} /> : null}
				</View>
			) : null}
		</View>
	);
...
function FileChangeActivity({ activity }: { activity: ConversationActivity }) {
	...
	return (
		<View style={useThemedStyles(makeStyles).activityWrap}>
			<ExpandableFileList
				title={activity.summary || `${files.length} changed files`}
				files={files}
				fallbackPatch={activity.detail?.patchOutput}
				fallbackPatchTruncated={activity.detail?.patchOutputTruncated}
				live={activity.status === "running"}
			/>
		</View>
	);
}

function ExpandableFileList({
	title,
	files,
	fallbackPatch,
	fallbackPatchTruncated,
	live,
}: {
	title: string;
	files: ReturnType<typeof fileChanges>;
	fallbackPatch?: string;
	fallbackPatchTruncated?: boolean;
	live?: boolean;
}) {
	const t = useTheme();
	const styles = useThemedStyles(makeStyles);
	...
	return (
		<View>
			...
			{open ? (
				<View style={styles.activityDetail}>
					{files.map((file) => (
						<FileChangeRow key={`${file.oldPath ?? ""}:${file.path}`} file={file} live={live} />
					))}
					{fallbackPatch ? <PatchBlock patch={fallbackPatch} truncated={fallbackPatchTruncated} /> : null}
				</View>
			) : null}
		</View>
	);
		...
		<View>
			<Pressable disabled={!hasPatch} onPress={() => setOpen((value) => !value)} style={styles.fileRow}>
				<Text
					style={[
						styles.fileMark,
						{ color: file.status === "deleted" ? t.red : file.status === "added" ? t.green : t.blue },
					]}
				>
					{mark}
				</Text>
				<Text selectable numberOfLines={2} style={styles.filePath}>
					{file.oldPath ? `${file.oldPath} → ${file.path}` : file.path}
				</Text>
				<Text style={styles.fileStat}>
					+{file.additions} −{file.deletions}
				</Text>
				{hasPatch ? <Feather name={open ? "chevron-up" : "chevron-right"} size={11} color={t.textFaint} /> : null}
			</Pressable>
			{open && file.patch ? <PatchBlock patch={file.patch} truncated={file.patchTruncated} /> : null}
		</View>
	);
}
...
function PatchBlock({ patch, truncated }: { patch: string; truncated?: boolean }) {
	...
	return (
		<View>
			...
			>
				<HighlightedCodeText code={patch} language="diff" style={styles.output} />
			</Pressable>
			{truncated ? (
				<Text style={[styles.partial, { color: t.amber }]}>
					This patch is longer than AO stores. The complete change remains in the worktree.
				</Text>
			) : null}
		</View>
	);
		...
		<View style={styles.planCard}>
			...
			{open ? (
				<View style={styles.planBody}>
					{activity.detail?.explanation ? <Text style={styles.detailCopy}>{activity.detail.explanation}</Text> : null}
					{steps.map((step, index) => (
						<View key={index} style={styles.planStep}>
							<Feather
								name={step.status === "completed" ? "check-circle" : "circle"}
								size={14}
								color={step.status === "completed" ? t.green : step.status === "in_progress" ? t.orange : t.textFaint}
							/>
							<Text style={[styles.planStepText, step.status === "completed" && styles.planDone]}>{step.text}</Text>
						</View>
					))}
					{!steps.length ? <Text style={styles.detailCopy}>{activity.detail?.text || activity.summary}</Text> : null}
				</View>
			) : null}
		</View>
	);
...
function ActivityTree({ node }: { node: ActivityNode }) {
	...
	return (
		<View>
			...
			{node.children.length ? <NestedAgentRun nodes={node.children} /> : null}
		</View>
	);
}
	...
	return (
		<View style={styles.subagent}>
			...
			>
				...
				<Text style={styles.subagentLabel}>
	...
	return (
		<View style={styles.turnWrap}>
			...
			<View style={styles.turnLine}>
				...
				<Text style={[styles.turnState, turn.state === "failed" && { color: t.red }]}>
			...
			{confirming ? (
				<View style={styles.rollbackConfirm}>
					...
					<Text style={styles.rollbackCopy}>
			...
			{open ? (
				<View style={styles.planBody}>
					...
					{turn.plan?.steps.map((step, index) => (
						<View
							key={index}
							accessibilityLabel={`${step.status.replace("_", " ")}: ${step.text}`}
							style={styles.planStep}
						>
							<Feather
								name={
									step.status === "completed" ? "check-circle" : step.status === "in_progress" ? "circle" : "circle"
								}
								size={14}
								color={step.status === "completed" ? t.green : step.status === "in_progress" ? t.orange : t.textFaint}
							/>
	...
	return (
		<View style={styles.planCard}>
			<Pressable style={styles.planHeader} onPress={() => setOpen((value) => !value)}>
				...
				{turn.state === "running" ? (
					<>
						<Text style={styles.planLive}>GROWING</Text>
			...
			{open ? (
				<View style={styles.fileBody}>
					{files.map((file) => (
						<View key={`${file.oldPath}:${file.path}`} style={styles.fileRow}>
							<Text
								style={[
									styles.fileMark,
									{ color: file.status === "deleted" ? t.red : file.status === "added" ? t.green : t.blue },
					...
					{turn.diff?.truncated ? (
						<Text style={[styles.partial, { color: t.amber }]}>
							This turn changed more files than AO lists here. Open the worktree shell for the complete diff.
						</Text>
					) : null}
				</View>
			) : null}
...
function ApprovalCard({
	...
	return (
		<View style={[styles.requestCard, pending && { borderColor: t.amber }]}>
			<View style={styles.requestTitle}>
				<Feather name="shield" size={15} color={pending ? t.amber : t.textTertiary} />
				<Text style={styles.requestHeading}>{pending ? "Approval required" : "Approval resolved"}</Text>
				{activity.requestId ? <Text style={styles.requestId}>req {activity.requestId}</Text> : null}
			</View>
			{activity.detail?.reason ? <Text style={styles.requestCopy}>{activity.detail.reason}</Text> : null}
			<Text selectable style={styles.requestCommand}>
				...
				activity.decisions?.length ? (
					<View style={styles.actions}>
						{activity.decisions.map((decision, index) => (
							<Action
								key={decision.id}
								label={submitting === decision.id ? "Sending…" : decision.label}
								primary={index === 0}
								disabled={busy || Boolean(submitting) || !activity.requestId}
								onPress={() => {
									setSubmitting(decision.id);
									setSubmitError(undefined);
									void onDecide(activity.requestId ?? "", decision.id)
					...
					<Text style={[styles.partial, { color: t.amber }]}>
						The agent offered no decisions AO can present. Open diagnostics from the host.
					</Text>
				)
			) : (
				<Text style={styles.partial}>Already answered. This card is kept for the record.</Text>
			)}
			{submitError ? (
...
function UserInputCard({
	...
	const [values, setValues] = useState<Record<string, unknown>>(() =>
		Object.fromEntries(
			Object.entries(schema?.properties ?? {}).map(([key, property]) => [key, initialInputValue(property)]),
		),
	);
	...
	const resolve = async (action: "accept" | "decline" | "cancel", content?: Record<string, unknown>) => {
		...
		setSubmitError(undefined);
		try {
			await onResolve(activity.requestId, action, content);
		} catch (cause) {
	...
	const submit = () => {
		...
		if (missing.length) {
			setValidationError(`Complete ${missing.join(", ")} before continuing.`);
			return;
		}
		for (const [name, property] of Object.entries(schema?.properties ?? {})) {
			const problem = validateInput(property, values[name]);
			if (problem) {
				setValidationError(`${property.title || humanizeInputName(name)} ${problem}.`);
				return;
			}
		}
	...
	return (
		<View style={[styles.requestCard, pending && { borderColor: t.blue }]}>
			<View style={styles.requestTitle}>
				<Feather name="message-square" size={15} color={pending ? t.blue : t.textTertiary} />
				<Text style={styles.requestHeading}>{schema?.title || (pending ? "Agent needs input" : "Input resolved")}</Text>
			</View>
			...
			{pending && activity.detail?.inputMode === "url" ? (
				<View style={styles.urlBox}>
					<Text selectable style={styles.urlText}>
						{url?.href ?? "The provider supplied an unsafe or invalid URL."}
					</Text>
				</View>
			) : null}
			{pending && activity.detail?.inputMode !== "url" && schema?.properties ? (
			...
			{pending && activity.requestId ? (
				<View style={styles.actions}>
					<Action label="Cancel" disabled={busy || submitting} onPress={() => void resolve("cancel")} />
					<Action
						label={activity.detail?.inputMode === "url" ? "Decline" : "Skip"}
						disabled={busy || submitting}
						onPress={() => void resolve("decline")}
					/>
					{activity.detail?.inputMode === "url" ? (
						<Action
							label={submitting ? "Opening…" : "Open link"}
							primary
							disabled={busy || submitting || !url}
							onPress={() => {
								if (!url) return;
				...
				<Text style={[styles.partial, { color: t.amber }]}>
					This request has no provider identity, so AO cannot answer it safely. Open diagnostics on the host.
				</Text>
			) : (
				<Text style={styles.partial}>Already answered. This card is kept for the record.</Text>
			)}
					...
					{options.map((choice) => {
						...
						return (
							<Action
								...
								primary={checked}
								onPress={() =>
									onChange(multi ? toggleInputValue(Array.isArray(value) ? value : [], choice.value) : choice.value)
								}
							/>
						);
					})}
	...
	return (
		<View style={styles.field}>
			...
			<TextInput
				...
				onChangeText={(text) =>
					onChange(property.type === "number" || property.type === "integer" ? (text === "" ? "" : Number(text)) : text)
				}
				keyboardType={property.type === "number" || property.type === "integer" ? "numeric" : "default"}
...
function CompactionMarker({ activity }: { activity: ConversationActivity }) {
	...
	return (
		<View style={styles.compaction}>
			<View style={styles.ruleHalf} />
			<Feather name="archive" size={12} />
			<Text style={styles.compactionText}>
				HISTORY COMPACTED{activity.detail?.tokensReclaimed ? `  −${formatTokens(activity.detail.tokensReclaimed)}` : ""}
				{after && window ? `  ${Math.round((after / window) * 100)}% FULL` : ""}
			</Text>
			<View style={styles.ruleHalf} />
		</View>
	);
...
function ErrorActivity({ activity }: { activity: ConversationActivity }) {
	...
	const styles = useThemedStyles(makeStyles);
	return (
		<View style={[styles.errorCard, { borderColor: t.tintRed }]}>
			<Feather name="alert-triangle" size={14} color={t.red} />
			<View style={{ flex: 1 }}>
				<Text style={styles.errorTitle}>{activity.summary || "Agent error"}</Text>
				{activity.detail?.error || activity.detail?.message ? (
					<Text selectable style={styles.errorCopy}>
						{String(activity.detail.error ?? activity.detail.message)}
					</Text>
				) : null}
			</View>
		</View>
...
function EmptyConversation({ harness, controller }: { harness: string; controller: string }) {
	...
	return (
		<View style={styles.empty}>
			<View style={[styles.emptyIcon, { backgroundColor: t.tintBlue }]}>
				<Feather name="message-square" size={20} color={t.blue} />
			</View>
			<Text style={styles.emptyTitle}>
				{controller === "connecting" ? "Connecting to the agent…" : "Start the conversation"}
			</Text>
			<Text style={styles.emptyCopy}>
				This {harness || "agent"} session works in its own AO worktree. Ask it to inspect, change, test, or explain
				anything there.
			</Text>
		</View>
	);
	...
	return (
		<View style={styles.fileBody}>
			{files.map((file, index) => (
				<Text key={index} selectable style={styles.filePath}>
					• {typeof file === "string" ? file : printable(file)}
				</Text>
			))}
		</View>
	);
...
function fileChanges(value: unknown): FileChange[] {
	...
	return value.flatMap((entry): FileChange[] => {
		...
		const status = ["added", "modified", "deleted", "renamed"].includes(String(file.status))
			? (String(file.status) as FileChange["status"])
			: "modified";
		return [
			{
				path: file.path,
				...
				patchTruncated: file.patchTruncated === true,
			},
		];
	});
}
...
function activityMeta(activity: ConversationActivity): {
	icon: keyof typeof Feather.glyphMap;
	prefix?: string;
	...
	switch (activity.activityKind) {
		case "command":
			return { icon: "terminal", color: (t) => (activity.status === "failed" ? t.red : t.textTertiary) };
		case "file_change":
			return { icon: "edit-3", prefix: "Changed", color: (t) => t.blue };
		case "mcp_tool":
			return {
				icon: "tool",
				prefix: activity.detail?.server ? `${activity.detail.server} ·` : "MCP ·",
				color: (t) => t.purple,
			};
		case "auto_review":
			return { icon: "shield", prefix: "Reviewed", color: (t) => t.green };
		default:
			return { icon: "activity", color: (t) => t.textTertiary };
	}
}

function elapsed(start?: string, end?: string): string | undefined {
