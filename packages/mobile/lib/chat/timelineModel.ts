...
export type ConversationTimelineRenderPlan =
	{ kind: "empty"; inverted: false; groups: [] } | { kind: "list"; inverted: true; groups: ConversationGroup[] };
...
export function conversationMarkers(snapshot: ConversationSnapshot): ConversationMarker[] {
	return groupConversationByTurn(snapshot).map((group) => {
		const human = group.items.find(
			(item) => item.kind === "message" && item.role === "user" && item.origin === "human",
		);
		const assistant = [...group.items]
			.reverse()
...
export function canRollbackTurn(snapshot: ConversationSnapshot, turn: ConversationTurn): boolean {
	return (
		snapshot.capabilities?.includes("rollback") === true &&
...
export function activityStartsExpanded(activity: ConversationActivity): boolean {
	...
	const liveBody =
		activity.status === "running" && Boolean(detail?.output || detail?.result || detail?.error || detail?.patchOutput);
