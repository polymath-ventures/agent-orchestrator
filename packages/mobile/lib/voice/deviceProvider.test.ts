...
describe("transcript delivery", () => {
	...
	it("keeps earlier speech when Android rolls over to a new segment with no final", async () => {
		...
		expect(h.onFinal).toHaveBeenCalledExactlyOnceWith("add a test for the pairing flow then run the linter");
	});
...
describe("lifecycle", () => {
	...
	it("reports a synchronous start failure through onError", async () => {
		...
		const spy = vi.spyOn(mod.ExpoSpeechRecognitionModule, "start").mockImplementationOnce(() => {
