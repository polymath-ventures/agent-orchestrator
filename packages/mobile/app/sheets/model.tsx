...
export default function ModelSheetRoute() {
	...
	const {
		...
		projectId = "",
		selected = "",
	} = useLocalSearchParams<{ resultKey?: string; agentId?: string; projectId?: string; selected?: string }>();
	...
	useEffect(() => {
		...
		let cancelled = false;
		getAgentModels(config, agentId, projectId)
			.then((value) => {
				if (!cancelled) {
					setCatalog(value);
					setError(undefined);
	...
	return (
		<ModelPickerSheet
			...
			error={error}
			onClose={() => router.back()}
			onSelect={(value) => takeSheetResult<string>(resultKey)?.(value)}
			onRefresh={() => {
				if (!config || !agentId || refreshing) return;
				setRefreshing(true);
				refreshAgentModels(config, agentId, projectId)
					.then((value) => {
						setCatalog(value);
						setError(undefined);
					})
