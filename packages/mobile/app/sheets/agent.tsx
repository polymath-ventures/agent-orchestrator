...
export default function AgentSheetRoute() {
	...
	const { config } = useApp();
	const { resultKey, selected, allowed, mode } = useLocalSearchParams<{
		resultKey?: string;
		selected?: string;
		allowed?: string;
		mode?: string;
	...
	const agents = useMemo(
		() => rankAgents(catalog).filter((agent) => !allowedSet || allowedSet.has(agent.id)),
		[catalog, allowedSet],
	);
	...
	return (
		<AgentPickerSheet
			...
			subtitle={
				mode === "chat"
					? "Installed agents with a structured Chat controller."
					: "The CLI that runs this terminal session."
			}
		/>
	);
}
