...
export class CloudClient {
	...
	async listAgents(orgId: string, options: RequestOptions = {}): Promise<AgentProfile[]> {
		const response = await this.request<{ agents: AgentProfile[] }>(this.orgPath(orgId, "/agents"), options);
		return response.agents;
	}

	listProjects(orgId: string, options: PaginationOptions = {}): Promise<ProjectPage> {
		return this.request(
	...
	updateProject(
		...
		return this.request(this.orgPath(orgId, `/projects/${encodeURIComponent(projectId)}`), {
			method: "PATCH",
			body: input,
			signal: options.signal,
		});
	...
	deleteProject(orgId: string, projectId: string, options: RequestOptions = {}): Promise<DeleteProjectResponse> {
		return this.request(this.orgPath(orgId, `/projects/${encodeURIComponent(projectId)}`), {
			method: "DELETE",
			signal: options.signal,
		});
	...
	getGitHubUserConnection(options: RequestOptions = {}): Promise<GitHubUserConnection> {
		return this.request("/api/cloud/v1/github/user", options);
	}

	startGitHubUserAuthorization(options: RequestOptions = {}): Promise<GitHubUserAuthorizationStart> {
	...
	async listGitHubInstallations(orgId: string, options: RequestOptions = {}): Promise<GitHubInstallation[]> {
	...
	startGitHubInstallation(orgId: string, options: RequestOptions = {}): Promise<GitHubInstallationStart> {
	...
	syncGitHubInstallation(
		...
		return this.request(this.orgPath(orgId, `/github/installations/${encodeURIComponent(installationId)}/sync`), {
			method: "POST",
			signal: options.signal,
		});
	}
	...
	disconnectGitHubInstallation(
		...
		return this.request(this.orgPath(orgId, `/github/installations/${encodeURIComponent(installationId)}/disconnect`), {
			method: "POST",
			signal: options.signal,
		});
	}

	listGitHubRepositories(orgId: string, options: PaginationOptions = {}): Promise<GitHubRepositoryPage> {
		return this.request(
	...
	listSessions(orgId: string, options: PaginationOptions & { projectId?: string } = {}): Promise<SessionPage> {
		return this.request(
	...
	getSession(orgId: string, sessionId: string, options: RequestOptions = {}): Promise<{ session: Session }> {
		return this.request(this.orgPath(orgId, `/sessions/${encodeURIComponent(sessionId)}`), options);
	}
	...
	deleteSession(orgId: string, sessionId: string, options: RequestOptions = {}): Promise<DeleteSessionResponse> {
		return this.request(this.orgPath(orgId, `/sessions/${encodeURIComponent(sessionId)}`), {
			method: "DELETE",
			signal: options.signal,
		});
	}
	...
	listSessionPullRequests(
		...
		return this.request(this.orgPath(orgId, `/sessions/${encodeURIComponent(sessionId)}/pull-requests`), options);
	}

	getSessionReviewState(orgId: string, sessionId: string, options: RequestOptions = {}): Promise<SessionReviewState> {
		return this.request(this.orgPath(orgId, `/sessions/${encodeURIComponent(sessionId)}/reviews`), options);
	}
	...
	sendMessage(
		...
		return this.request(this.orgPath(orgId, `/sessions/${encodeURIComponent(sessionId)}/messages`), {
			method: "POST",
			body: { text },
			idempotencyKey: options.idempotencyKey,
			signal: options.signal,
	...
	cancelTurn(
		...
		return this.request(
			this.orgPath(orgId, `/sessions/${encodeURIComponent(sessionId)}/turns/${encodeURIComponent(turnId)}/cancel`),
			{
				method: "POST",
				idempotencyKey: options.idempotencyKey,
				signal: options.signal,
	...
	replayEvents(orgId: string, sessionId: string, options: EventReplayOptions = {}): Promise<ClientEventPage> {
		const path = this.orgPath(orgId, `/sessions/${encodeURIComponent(sessionId)}/chat-events`);
		return this.request(
			this.withQuery(path, {
	...
	async *streamEvents(
		...
		const endpoint = this.orgPath(orgId, `/sessions/${encodeURIComponent(sessionId)}/events`);
		...
		while (!options.signal?.aborted) {
			...
			try {
				...
				if (!response.body) {
					throw this.invalidResponse(response.status, "Cloud event stream returned no response body.");
				}
	...
	createTerminalTicket(
		...
		return this.request(this.orgPath(orgId, `/sessions/${encodeURIComponent(sessionId)}/terminal-ticket`), {
			method: "POST",
			body: { kind },
			signal: options.signal,
		});
	...
	terminalUrl(ticket: string, options: { after?: number; kind?: TerminalKind } = {}): string {
	...
	listWorkspaceFiles(
		...
		const endpoint = this.orgPath(orgId, `/sessions/${encodeURIComponent(sessionId)}/workspace/files`);
		return this.request(
	...
	readWorkspaceFile(
		...
		const endpoint = this.orgPath(orgId, `/sessions/${encodeURIComponent(sessionId)}/workspace/file`);
	...
	writeWorkspaceFile(
		...
		return this.request(this.orgPath(orgId, `/sessions/${encodeURIComponent(sessionId)}/workspace/file`), {
			...options,
			method: "PUT",
			body: input,
		});
	...
	getWorkspaceDiff(orgId: string, sessionId: string, options: RequestOptions = {}): Promise<WorkspaceDiff> {
		return this.request(this.orgPath(orgId, `/sessions/${encodeURIComponent(sessionId)}/workspace/diff`), options);
	}

	async listProviderConnections(orgId: string, options: RequestOptions = {}): Promise<RedactedProviderConnection[]> {
		const response = await this.request<{
	...
	putAgentProviderConnection(
		...
		return this.request(this.orgPath(orgId, `/provider-connections/agents/${encodeURIComponent(provider)}`), {
			method: "PUT",
			body: input,
			signal: options.signal,
		});
	...
	async deleteAgentProviderConnection(
		...
		const response = await this.authorizedFetch(
			this.orgPath(orgId, `/provider-connections/agents/${encodeURIComponent(provider)}`),
			{
				method: "DELETE",
	...
	private async request<T>(path: string, options: JSONRequestOptions = {}): Promise<T> {
		...
		try {
			...
			throw this.invalidResponse(response.status, "Cloud API returned an invalid JSON response.");
		}
	}
...
export class WorkerClient {
	...
	bootstrap(input: WorkerBootstrapInput, options: RequestOptions = {}): Promise<WorkerBootstrapResponse> {
	...
	heartbeat(input: WorkerHeartbeatInput, options: RequestOptions = {}): Promise<WorkerHeartbeatResponse> {
	...
	publishEvent(input: WorkerEventInput, options: RequestOptions = {}): Promise<WorkerOKResponse> {
	...
	async claimTurn(options: RequestOptions = {}): Promise<WorkerTurn | null> {
		const response = await this.request<{ turn: WorkerTurn | null }>("/api/cloud/v1/worker/turns/claim", {
			method: "POST",
			body: {},
			signal: options.signal,
		});
	...
	getTurnCancellation(
		...
		return this.request(`/api/cloud/v1/worker/turns/${encodeURIComponent(turnId)}/cancellation?${query.toString()}`, {
			signal: options.signal,
		});
	}
	...
	getCredential(options: RequestOptions = {}): Promise<WorkerCredentialResponse> {
	...
	createCheckoutGrant(options: RequestOptions = {}): Promise<WorkerCheckoutGrantResponse> {
	...
	listChildren(options: PaginationOptions = {}): Promise<SessionPage> {
		const query = new URLSearchParams();
	...
	createChild(input: CreateWorkerChildInput, options: IdempotentRequestOptions): Promise<{ session: Session }> {
	...
	sendChildMessage(
		...
		return this.request(`/api/cloud/v1/worker/children/${encodeURIComponent(sessionId)}/messages`, {
			method: "POST",
			body: { text },
			idempotencyKey: options.idempotencyKey,
			signal: options.signal,
	...
	deleteChild(sessionId: string, options: RequestOptions = {}): Promise<DeleteSessionResponse> {
		return this.request(`/api/cloud/v1/worker/children/${encodeURIComponent(sessionId)}`, {
			method: "DELETE",
			signal: options.signal,
		});
	}

	async claimTransport(options: RequestOptions = {}): Promise<WorkerTransportRequest | null> {
		const response = await this.request<{
	...
	completeTransport(
		...
		return this.request(`/api/cloud/v1/worker/transport/${encodeURIComponent(requestId)}/complete`, {
			method: "POST",
			body: input,
			signal: options.signal,
		});
	...
	failTransport(
		...
		return this.request(`/api/cloud/v1/worker/transport/${encodeURIComponent(requestId)}/fail`, {
			method: "POST",
			body: input,
			signal: options.signal,
		});
	...
	ensureAgentTerminal(options: RequestOptions = {}): Promise<WorkerAgentTerminalResponse> {
	...
	publishTerminalOutput(
		...
		return this.request(`/api/cloud/v1/worker/terminals/${encodeURIComponent(terminalId)}/output`, {
			method: "POST",
			body: input,
			signal: options.signal,
		});
	...
	publishTerminalExit(
		...
		return this.request(`/api/cloud/v1/worker/terminals/${encodeURIComponent(terminalId)}/exit`, {
			method: "POST",
			body: input,
			signal: options.signal,
		});
	...
	private async unauthenticatedRequest<T>(path: string, options: JSONRequestOptions): Promise<T> {
...
async function waitForRetry(attempt: number, signal: AbortSignal | undefined): Promise<void> {
...
function toErrorEnvelope(response: Response, value: unknown): ErrorEnvelope {
	...
	return {
		error: typeof object.error === "string" ? object.error : response.statusText || "Request Failed",
		code: typeof object.code === "string" ? object.code : "HTTP_ERROR",
		message:
			typeof object.message === "string" ? object.message : `Cloud API request failed with status ${response.status}.`,
		requestId: typeof object.requestId === "string" ? object.requestId : (response.headers.get("x-request-id") ?? ""),
