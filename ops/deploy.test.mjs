...
test("deploy.sh keeps its load-bearing invariants", async () => {
	...
	// runs the extracted probe against fixture payloads. What that suite CANNOT
	// see is the shell wiring around the probe — it invokes python directly — so
	...
	assert.doesNotMatch(text, /<<'PY' 2>&1/);
