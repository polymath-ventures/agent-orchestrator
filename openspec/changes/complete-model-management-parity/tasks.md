...
- [ ] 2.1 Port the catalog, request-cache, bounded-concurrency, pin-verdict TTL/eviction, and three-way validation core from reference `backend/internal/service/agent/service.go:341-613,728-745`, replacing static-primary catalogs with an adapter catalog capability and visible fallbacks.
- [ ] 2.2 Add failing then passing tests for config-save validation, cache-only fail-open spawn validation, definitive cached rejection, per-pin timeout independence, force refresh, configured-pin merging, and bounded eviction.
- [ ] 2.3 Port `GET /agents/models` with `force` support from reference controller/spec-generator seams, enrich the read model with model labels, efforts, defaults, verification/fallback metadata, and register telemetry templates.
- [ ] 2.4 Regenerate OpenAPI and TypeScript schema output from the controller/spec generator; do not hand-edit generated API artifacts.
...
- [ ] 4.1 Wire the shared three-way validator into project config save without paid Claude prompts and into session creation before any session row, runtime, or worktree is created.
- [ ] 4.2 Wire cache-only `ValidateSpawnModel` into the resolved launch path so only a fresh definitive rejection blocks and missing/stale/unknown verdicts proceed with a loud warning.
...
- [ ] 4.5 Port `/agents/health` and the install/auth monitor with `AO_AGENT_HEALTH_INTERVAL`, transition logging/callbacks, actionable `missing`/`unauthorized` remedies, advisory `unknown`, and no session-notification persistence.
