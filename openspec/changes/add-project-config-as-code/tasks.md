...
- [ ] 2.1 Write a failing CLI test (stub-server pattern from `project_test.go`): `export <project>` issues `GET /api/v1/projects/{id}`, prints the config as canonical JSON, and exits zero; a missing project argument exits 2 (usageError) with no daemon call; a daemon 404 surfaces the error and exits nonzero.
