...
- [x] 1.4 Implement `DeriveSessionPrefix` in `backend/internal/domain/session_naming.go` — inputs are the project name, the project id, and the set of prefixes already in use; output is a lowercase alphanumeric prefix of at most 3 characters. Make 1.1–1.3 pass.
...
- [x] 3.3 Confirm the frontend settings form needs no change: verify the create response carries the derived prefix so the field renders populated, and add or extend web-mode coverage if the populated value is not already exercised.
