# sidebar-harness-indicator Specification

## Purpose

A session's display name deliberately does not say which harness is running it.
The `session-naming` grammar caps a name at 20 runes, and spending part of that
budget on the harness muddies the identity the name exists to carry. That left
an operator unable to tell a `claude-code` session from a `codex` or
`codex-fugu` one in the sidebar without opening it.

This capability is where the harness becomes visible instead: a small indicator
beside the per-session status dot, weighted like the dot rather than like the
name. Status (what the session is doing) and harness (what is running it) are
the two things worth knowing without opening a session, so they travel together
as one marker slot.

Two properties shape the requirements below. The indicator must resolve for
**every** harness the daemon can spawn — including one added after the mapping
was written — because a blank slot in a dense row reads as a broken row rather
than an unknown harness, so the fallback is total by construction rather than
enforced by a list that has to be kept in step. And the harness must reach
assistive technology through the row that contains the indicator: an indicator
carried by colour and shape alone is no indicator at all for some operators, and
a graphic cannot label itself inside a row control that already has an
accessible name.
