# codex-fugu-harness Specification

## Purpose

`codex-fugu` is a fork-only, Codex-compatible worker harness that launches the
`codex-fugu` binary while reusing Codex's flags, hooks, prompt delivery, and
activity model. It is served by the existing Codex adapter, parameterized by
manifest identity, binary name, and hook token, so a zero-valued adapter stays
plain Codex. The harness suppresses the wrapper's update prompt, reports activity
under its own hook token, and resolves authorization through the shared Codex
login. It is Polymath-internal and never ships upstream.
