# committed-config-drift Specification

## Purpose

Keep each tracked project's exported config as a committed, version-controlled
snapshot, and detect drift between those snapshots and live config on a schedule
— surfacing drift to the operator without self-healing. This is the fork-only
convenience layer on top of `project-config-as-code`'s export/diff CLI.
