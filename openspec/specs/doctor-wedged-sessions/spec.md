# doctor-wedged-sessions Specification

## Purpose

TBD - created by archiving change add-doctor-wedged-sessions. Update Purpose after archive.
...
**active** has been in that state, with no transition to any other state, for
longer than a fixed threshold. The warning SHALL name each such session, how
long it has been in that state, and a command the operator can run next, so it
...
keeps for each session, read over the existing loopback session listing. It
SHALL NOT inspect processes: no process-tree walk, no `ps`, no `tmux`, and no
other external process-inspection tool. It SHALL NOT introduce any
platform-specific tool invocation, so it behaves identically on macOS and Linux
by construction.
