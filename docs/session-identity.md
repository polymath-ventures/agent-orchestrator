...
| --- | ----------------------------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | AO session ID                 | Identity                    | Minted once by the SQLite store as `{project}-{num}-{generation}` (or `prime-{num}-{generation}`); persisted as `sessions.id`                               | `{num}` is monotonic within one database. The 128-bit lowercase-hex generation token is minted once in `daemon_settings` for that database generation, so a rebuilt database cannot reuse the complete ID. Pre-existing `{project}-{num}` IDs are never rewritten. |
...
`daemon_settings.session_id_generation` is a 32-character lowercase-hex token
(128 bits) minted when the migration initializes a database. It is read, not
