...
| 7   | tmux session                  | External identity namespace | New workers derive the runtime handle from the persisted namespace key                                                                                      | Keys at most 48 bytes remain verbatim. Longer keys retain a readable 31-byte head plus a 64-bit SHA-256 digest. Create and restart derive the same handle; lookup, attach, and destroy use the persisted opaque handle. Legacy rows retain their existing handles. |
...
unsafe character runs become `-`, the label alone is capped at 24 bytes, and an
empty result falls back to `work`. The stored form is
...
tmux has the one surface-specific shortening rule. A conforming key up to 48
bytes is used verbatim. A longer or nonconforming key is sanitized to a readable
head of at most 31 bytes and suffixed with `-` plus 16 lowercase hexadecimal
characters (the first 64 bits of SHA-256 over the complete key). The digest is
computed from the entire key, so shortening cannot discard the generation
token's collision protection.
