...
4. **Harness/agent setup & selection.** The agent-selection catalog, the per-role
   ...
   catalog itself. Worker spawn selection precedence is explicit spawn harness,
   configured `workerMix`, scalar `worker.agent`, then an automatic even split
   over installed+authorized worker harnesses, with any remainder assigned by
   lowest harness id first. On the backend the mix is resolved at spawn by
...
9. **Ops / SDLC infrastructure.** `ops/deploy.sh` + the web server + systemd /
   ...
   until the upstream hook-regeneration gap recorded on assets#255 is closed. Repo-owned client skill copies such as `sync-upstream` stay tracked for fresh clones; untracked sx-installed client copies are restored by running `/polypowers-init`.
