...
| `ao doctor` / `--json`        | Check config, data directory, DB-file presence, daemon state, live-session activity (sessions the daemon calls active that have gone silent), `git`, and (on Darwin/Linux) `tmux`; on Windows conpty is built in. |
