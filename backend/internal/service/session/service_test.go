...
func (f *fakeCommander) SwitchAgent(context.Context, domain.SessionID, sessionmanager.SwitchAgentConfig) (domain.AgentSwitch, error) {
	return domain.AgentSwitch{}, nil
}

func (f *fakeCommander) ListAgentSwitches(context.Context, domain.SessionID) ([]domain.AgentSwitch, error) {
	return nil, nil
}

func (f *fakeCommander) SubmitAgentHandoff(context.Context, domain.SessionID, domain.AgentSwitchID, domain.AgentGenerationID, json.RawMessage) (domain.AgentSwitch, error) {
	return domain.AgentSwitch{}, nil
}
...
func TestSpawnOrchestratorCleanRetireNoticeNamesCanonicalBranch(t *testing.T) {
	...
	if !strings.Contains(strings.ToLower(fc.sentMessages[0]), "canonical branch") {
		t.Fatalf("retire notice = %q, want canonical branch", fc.sentMessages[0])
	}
}
...
// service a true-nil ports.Tracker. Spawn must still create the session, keep the
// work item on it, and skip only the GitHub issue-context enrichment — not panic
// on a typed-nil tracker the way the pre-fix wiring did.
//
// The id is canonicalised even here (#298): canonicalisation reads the project's
// origin, not the tracker, and a record written without it would be invisible to
// intake's dedup the moment a token appeared.
func TestSpawnPreservesIssueIDWhenTrackerIsNil(t *testing.T) {
	...
	if fc.spawnedCfg.IssueID != "github:acme/repo#107" {
		t.Fatalf("IssueID = %q, want github:acme/repo#107", fc.spawnedCfg.IssueID)
	}
	if fc.spawnedCfg.IssueContext != "" {
...
func TestSpawnRejectsManualPrimeKind(t *testing.T) {
	...
	_, _, _, err := svc.Spawn(context.Background(), ports.SpawnConfig{ProjectID: "ao", Kind: domain.KindPrime})
	var e *apierr.Error
