...
// CreateSession owns session identity in this fork, so each store fixture records
// the generated ID instead of assuming the caller-supplied upstream ID survives.
var testSession domain.SessionID
...
func TestFreshProjectControllerRecordsNativeContextBoundary(t *testing.T) {
	...
	if boundary.Kind != domain.ActivityKindSystem || boundary.ProviderItemID != "ao-context-reset:"+string(replacement) {
...
func TestSendRefusedForTUISession(t *testing.T) {
	...
	tuiRecord, err := h.st.CreateSession(ctx, domain.SessionRecord{
		ProjectID: testProject,
