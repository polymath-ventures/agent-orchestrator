...
const (
	...
	KindConflict
	// KindForbidden is an authenticated request that is not allowed by a
	// daemon-owned business rule; it maps to 403.
	KindForbidden
)
...
func Forbidden(code, message string, details map[string]any) *Error {
	return New(KindForbidden, code, message, details)
}
