...
func TestWiring_StartSessionBuildsSessionService(t *testing.T) {
	...
	svc, reviewSvc, lc, err := startSession(context.Background(), cfg, rt, store, lcm, messenger, nil, nil, telemetryadapter.NoopSink{}, agents, nil, nil, nil, nil, nil, log)
	if err != nil {
...
func TestWiring_StartSessionSpawnsScratchWithoutGitRepo(t *testing.T) {
	...
	svc, _, _, err := startSession(context.Background(), cfg, runtime, store, lcm, messenger, nil, nil, telemetryadapter.NoopSink{}, agents, nil, nil, nil, nil, nil, log)
	if err != nil {
		t.Fatalf("startSession: %v", err)
	}
...
func TestStartSession_SpawnDoesNotPanicWhenNoTrackerToken(t *testing.T) {
	...
	svc, _, _, err := startSession(context.Background(), cfg, rt, store, lcm, messenger, nil, nil, telemetryadapter.NoopSink{}, agents, nil, nil, nil, nil, nil, log)
	if err != nil {
		t.Fatalf("startSession: %v", err)
	}

	// Spawn reaches withIssueDetails (and the tracker guard) before the manager
...
func TestStartSessionInjectsSharedSpawnModelValidator(t *testing.T) {
	...
	svc, _, _, err := startSession(ctx, config.Config{DataDir: t.TempDir()}, rt, store, lcm, messenger, validator, nil, telemetryadapter.NoopSink{}, agents, nil, nil, nil, nil, nil, log)
	if err != nil {
		t.Fatal(err)
	}

	_, _, _, err = svc.Spawn(ctx, ports.SpawnConfig{ProjectID: "models", Kind: domain.KindWorker, Prompt: "test"})
