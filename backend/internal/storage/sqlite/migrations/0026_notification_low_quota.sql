...
SELECT
    ...
    pr_url,
    CASE
        WHEN pr_url != '' THEN 'project:' || project_id || ':session:' || session_id || ':pr:' || pr_url
        ELSE 'project:' || project_id || ':session:' || session_id
    END,
    type,
    title,
    body,
...
CREATE UNIQUE INDEX idx_notifications_unread_dedupe
    ...
    WHERE status = 'unread' AND dedupe_key != '';
-- +goose StatementEnd
