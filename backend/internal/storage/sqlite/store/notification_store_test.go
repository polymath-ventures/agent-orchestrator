...
func TestNotificationStore_ListUnreadBeforePaginatesTimestampTies(t *testing.T) {
	...
	first, err := s.ListNotifications(ctx, domain.NotificationListUnread, time.Time{}, "", 100)
	if err != nil {
		t.Fatal(err)
	}
	...
	second, err := s.ListNotifications(ctx, domain.NotificationListUnread, first[99].CreatedAt, first[99].ID, 100)
	if err != nil {
		t.Fatal(err)
	}
