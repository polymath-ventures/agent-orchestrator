...
func TestMigration0090MovesReviewerSessionsIntoPerHarnessReviewRows(t *testing.T) {
	db, err := sql.Open("sqlite", "file:"+filepath.Join(t.TempDir(), "ao.db")+"?_pragma=busy_timeout(5000)")
	if err != nil {
		t.Fatalf("open sqlite: %v", err)
	}
...
func TestPrepareReviewPerHarnessMigrationRepairsApplied0090StaleReviewShape(t *testing.T) {
	db, err := sql.Open("sqlite", "file:"+filepath.Join(t.TempDir(), "ao.db")+"?_pragma=busy_timeout(5000)")
	if err != nil {
		t.Fatalf("open sqlite: %v", err)
	}
...
INSERT INTO goose_db_version (version_id, is_applied) VALUES (48, 1), (49, 1), (89, 1), (90, 1);
