...
func (e *Engine) Trigger(ctx stdctx.Context, workerID domain.SessionID, override domain.ReviewerHarness) (TriggerResult, error) {
	...
	reviewRow, err = e.upsertReview(ctx, worker, eagerHarness, reviewRow.ReviewerHandleID, reviewRow.AgentSessionID, now)
	if err != nil {
		return TriggerResult{}, err
	}
	...
	for _, reviewState := range reviews {
		...
		runHarness := harness
		if override == "" && reviewState.Status == ReviewStateChangesRequested && reviewState.LatestRun != nil && reviewState.LatestRun.Harness == "" {
			runHarness = domain.ReviewerClaudeCode
		}
		run := domain.ReviewRun{
			ID:        e.newID(),
	...
	handleID := ""
	canNotify := reviewRow.ReviewerHandleID != "" && prevHarness == harness && e.launcher.Reusable(harness) &&
		reviewerPaneReusable(reviewRow, hadRunningReviewer)
	if canNotify {
	...
	if handleID == "" {
		...
		launch, err := e.launcher.Spawn(ctx, reviewLaunchSpec(worker, harness, agentConfig, created[0], queue, 0, reviewerRules, reviewRow.AgentSessionID, runs))
		if err != nil {
		...
		if err := e.launcher.Notify(ctx, handleID, reviewLaunchSpec(worker, harness, agentConfig, created[0], queue, 0, reviewerRules, reviewRow.AgentSessionID, runs)); err != nil {
			return TriggerResult{}, failRuns(0, fmt.Errorf("notify reviewer: %w", err))
		}
	}
	reviewRow, err = e.upsertReview(ctx, worker, harness, handleID, agentSessionID, now)
	if err != nil {
		return TriggerResult{}, err
	}
	for i := range created {
...
func reviewerPaneReusable(reviewRow domain.Review, hadRunningReviewer bool) bool {
	return strings.TrimSpace(reviewRow.ReviewerHandleID) != "" || hadRunningReviewer
}
...
func reviewLaunchSpec(worker domain.SessionRecord, harness domain.ReviewerHarness, agentConfig ports.AgentConfig, run domain.ReviewRun, queue []ports.ReviewTask, index int, reviewerRules, agentSessionID string, previousRuns []domain.ReviewRun) LaunchSpec {
	return LaunchSpec{
		RunID:           run.ID,
		BatchID:         run.BatchID,
