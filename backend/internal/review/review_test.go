...
func TestTriggerRespawnsWhenReviewerHarnessChanged(t *testing.T) {
	...
	// Live pane exists (alive), but the worker/project now resolves to codex
	// while the pane was launched under claude-code.
	launcher := &fakeLauncher{alive: true, handle: "review-mer-1"}
	eng := newEngineForTest(store, fakeSessions{rec: liveWorker(), ok: true}, prAt("sha1"), fakeProjects{}, launcher)
...
func TestTriggerKeepsHarnessWhenNothingCreated(t *testing.T) {
	...
	// Live claude-code pane; the worker/project now resolves to codex.
	launcher := &fakeLauncher{alive: true, handle: "review-mer-1"}
	eng := newEngineForTest(store, fakeSessions{rec: liveWorker(), ok: true}, prAt("sha1"), fakeProjects{}, launcher)
	...
	if store.review.Harness != domain.ReviewerClaudeCode {
		t.Fatalf("recorded harness = %q, want claude-code preserved (no respawn happened)", store.review.Harness)
	}
}
...
func TestTriggerRespawnsOnNextCommitAfterHarnessSwitchWithNoRun(t *testing.T) {
	...
	// the worker now resolves to codex but the live pane is still claude-code.
	...
	// respawn under codex, not Notify the stale claude-code pane.
