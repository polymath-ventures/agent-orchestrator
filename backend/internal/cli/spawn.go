...
func newSpawnCommand(ctx *commandContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if opts.kind == "orchestrator" && opts.harness == "" {
			...
			// Preflight only a harness the CLI actually knows. When a worker
			// spawn is unpinned the daemon picks the harness, so the client
			// cannot check readiness against a harness it would have to guess.
	...
	f.BoolVar(&opts.force, "force", false, "Spawn even while the project or fleet is paused (overrides the pause guard)")
	return cmd
}
