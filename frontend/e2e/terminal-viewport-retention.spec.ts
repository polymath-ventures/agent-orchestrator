...
import { installFakeTerminalMux, type FakeTerminalMuxStats } from "./support/fake-terminal-mux";
...
async function observeNextReveal(container: Locator): Promise<void> {
	await container.evaluate((element) => {
		...
		const sampleFrame = () => {
			const phase = (element as HTMLElement).dataset.terminalActivationPhase ?? "missing";
...
test.describe("retained terminal viewport", () => {
	test("retains the first of six live sessions with zero reopen and reveals its latest output", async ({ page }) => {
		...
		expect(firstRevealFrames.filter((phase) => phase === "revealed").length).toBeGreaterThanOrEqual(2);
		expect(firstRevealFrames.indexOf("revealed")).toBeLessThan(firstRevealFrames.indexOf("visible"));
		...
		const fontSizeBefore = Number((await page.getByText(/^\d+px$/).textContent())?.replace("px", ""));
		...
		const activationResizePhases = (await muxStats(page)).resizePhases[handleA]?.slice(aResizesBeforeReturn);
	...
	test("suppresses parked resize and scroll storms during rapid A-B-A switching", async ({ page }) => {
		await installHarness(page);
		const viewport = activeViewport(page);
		await expect
			.poll(() => viewport.evaluate((element) => element.scrollHeight - element.clientHeight))
		...
		expect((await muxStats(page)).resizes[handleA]?.length ?? 0).toBe(beforeResize.resizes[handleA]?.length ?? 0);
		await page.setViewportSize({ width: 1280, height: 800 });
		...
		await openSession(page, sessionA.title);
		await expect.poll(async () => (await muxStats(page)).resizes[handleA]?.length ?? 0).toBe(aResizesBeforeReturn);
	...
	test("handles empty and short replay, blocks hidden input, and retains xterm through reconnect", async ({ page }) => {
		...
		await expect.poll(async () => (await muxStats(page)).inputs[handleB]?.join("") ?? "").toContain("b-input");
