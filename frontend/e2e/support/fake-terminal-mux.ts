...
export type FakeTerminalMuxStats = {
	...
	openFrames: Record<string, { cols?: number; phase: string | null; rows?: number }[]>;
 ...
 */
export async function installFakeTerminalMux(page: Page, initialReplay: Record<string, string>): Promise<void> {
	await page.addInitScript((replayByHandle) => {
		...
		const openFrames: Record<string, { cols?: number; phase: string | null; rows?: number }[]> = {};
		...
		const terminalPhase = (handleId: string): string | null =>
			Array.from(document.querySelectorAll<HTMLElement>("[data-terminal-cache-key]")).find((element) =>
		...
		window.__aoFakeTerminalMux = {
			...
			stats: () => ({
				...
				sockets: sockets.filter((socket) => socket.isTerminalMux && socket.readyState < FakeWebSocket.CLOSED).length,
				writers: sockets.reduce(
					(total, socket) =>
						total + (socket.isTerminalMux && socket.readyState < FakeWebSocket.CLOSED ? socket.attachmentCount : 0),
					0,
				),
			}),
		};
