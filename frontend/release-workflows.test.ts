...
describe("desktop release workflows", () => {
	for (const workflow of ["build-artifacts.yml", "frontend-release.yml"]) {
		it(`${workflow} requires the WorkOS client ID`, async () => {
			...
			expect(contents).toContain("VITE_WORKOS_CLIENT_ID: ${{ vars.VITE_WORKOS_CLIENT_ID }}");
			expect(contents).toContain("Repository variable VITE_WORKOS_CLIENT_ID is required");
		});
	}
});
