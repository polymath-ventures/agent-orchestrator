...
import { decideRelocation, inspectInstalledBundle, installedBundlePath } from "./main/relocation";
...
import { TRAY_RENDERER_READY_CHANNEL, TRAY_SET_ATTENTION_STATE_CHANNEL } from "./shared/tray";
import {
	type DaemonProbe,
	expectedDaemonPort,
	parseDaemonProbe,
...
import { handleCloudDeepLink, installCloudIPC, registerCloudProtocol, showCloudSignInFailure } from "./main/cloud-auth";
...
ipcMain.handle("menu:action", (_event, action: string) => {
	...
	const wc = (focused && focused !== shell ? focused : browserViewHost?.getLastFocusedPanelContents()) ?? shell;
	if (!wc) return;
	switch (action) {
		case "edit.undo":
			return wc.undo();
...
function cloudDataDir(): string {
	return isDev ? path.join(os.homedir(), ".ao", DEV_STATE_SUBDIR) : path.join(os.homedir(), ".ao");
}
