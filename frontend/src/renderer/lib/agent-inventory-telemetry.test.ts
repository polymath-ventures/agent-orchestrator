...
type TestAgentInfo = Omit<NonNullable<AgentCatalog["supported"]>[number], "reviewerCapable"> & {
	reviewerCapable?: boolean;
};
type TestAgentCatalog = Omit<Partial<AgentCatalog>, "installed" | "authorized" | "supported"> & {
	installed?: TestAgentInfo[];
	authorized?: TestAgentInfo[];
	supported?: TestAgentInfo[];
};

const catalog = (over: TestAgentCatalog = {}): AgentCatalog =>
