...
import type { ConversationItem, ConversationSnapshot, ConversationTurn } from "../types/conversation";
	...
	turns: [
		{
			...
			diff: {
				files: [
					...
					{
						path: "backend/internal/adapters/chatdriver/codexappserver/diff.go",
						additions: 128,
						deletions: 0,
						status: "added",
					},
					...
					{
						path: "backend/internal/chat/normalize.go",
						oldPath: "backend/internal/chat/translate.go",
						additions: 4,
						deletions: 4,
						status: "renamed",
					},
		...
		},
		{
			...
			diff: {
				files: [
					{
						path: "backend/internal/storage/sqlite/queries/conversations.sql",
						additions: 27,
						deletions: 0,
						status: "modified",
					},
	...
	items: [
		...
		},
		{
			...
			detail: {
				...
				rawCommand: "/bin/zsh -lc \"ao spawn --project agent-orchestrator-1 --name http-layer --prompt '…'\"",
				cwd: "/Users/dhruv/.ao/data/worktrees/agent-orchestrator-1/ao-14",
...
export const chatFixtureSettled: ConversationSnapshot = {
	...
	turns: chatFixture.turns.map((turn) =>
		turn.state === "running" ? { ...turn, state: "completed" as const, completedAt: t(39, 40) } : turn,
	),
};
...
export const chatFixtureReasoningEmpty: ConversationSnapshot = {
	...
	items: chatFixtureSettled.items.map((item) =>
		item.kind === "activity" && item.activityKind === "reasoning" ? { ...item, detail: undefined } : item,
	),
};
