 ...
 */
export function useStableList<T>(list: T[], keyOf: (entry: T) => string, equal: (a: T, b: T) => boolean): T[] {
...
export function sameContent(a: unknown, b: unknown): boolean {
	...
	return keys.every((key) => sameContent((a as Record<string, unknown>)[key], (b as Record<string, unknown>)[key]));
}
