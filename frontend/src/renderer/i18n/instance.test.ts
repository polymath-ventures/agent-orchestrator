	...
	it("keeps locale catalogs covering every English key with non-empty values", () => {
		...
		for (const locale of APP_LOCALES) {
			...
			for (const key of enKeys) {
				...
				expect(String(catalog[key as keyof typeof catalog] ?? "").length, `${locale}.${key} empty`).toBeGreaterThan(0);
			}
		}
	});
		...
		for (const locale of APP_LOCALES) {
			...
			for (const base of pluralBases) {
				for (const category of requiredCategories) {
					expect(`${base}_${category}` in allCatalogs[locale], `${locale} is missing ${base}_${category}`).toBe(true);
				}
			}
		}
	});
