...
import { agentModelsQueryKey, agentModelsQueryOptions, revalidateAgentModels } from "../hooks/useAgentModelsQuery";
...
export function TaskComposer({
	...
	const createTask = useCallback(
		async (input: CreateTaskInput): Promise<string> => {
			...
			try {
				...
				if (error) {
					throw new TaskCreateError(apiErrorMessage(error, t("newTask.unableToStart")), apiErrorCode(error));
				}
	...
	const revalidationQuery = useQuery({
		queryKey: ["agent-model-revalidation", selectedAgent, projectId ?? "", modelCatalogQuery.data?.validatedAt ?? ""],
	...
	useEffect(() => {
		if (revalidationQuery.data) {
			queryClient.setQueryData(agentModelsQueryKey(selectedAgent, projectId ?? ""), revalidationQuery.data);
		}
	...
	const defaultModelForSelectedAgent = projectModelForSelectedAgent || (catalogUsesModes ? "" : catalogDefaultOption);
	const defaultModeForSelectedAgent = projectModeForSelectedAgent || (catalogUsesModes ? catalogDefaultOption : "");

	const selectedAgentLabel = agentCatalog?.supported?.find((item) => item.id === selectedAgent)?.label || selectedAgent;
	const requiresTuiFallback =
		selectedAgent !== "" && settings?.defaultSessionMode === "chat" && !settings.chatHarnesses.includes(selectedAgent);
	...
	return (
		<TaskComposerView
			...
			model={{
				...
				loading: selectedAgent !== "" && modelCatalogQuery.isFetching && modelCatalogQuery.data === undefined,
				refreshing: false,
				onModelChange: (value) => {
					setModel(value);
					setMode("");
...
function DesktopAgentControl(control: TaskComposerAgentControl) {
	const withReviewerCapable = (items?: TaskComposerAgentControl["authorized"]) =>
		items?.map((item) => ({ ...item, reviewerCapable: false }));
	return (
		<RequiredAgentField
			{...control}
			authorized={withReviewerCapable(control.authorized)}
			installed={withReviewerCapable(control.installed)}
			supported={withReviewerCapable(control.supported)}
			variant="chip"
