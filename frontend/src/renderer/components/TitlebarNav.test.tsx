...
vi.mock("../lib/runtime-environment", () => ({
	isMacDesktopChrome: () => true,
}));

function renderTitlebarNav(props: React.ComponentProps<typeof TitlebarNav> = {}) {
	return render(
		<SidebarProvider
			open={useUiStore.getState().isSidebarOpen}
			onOpenChange={(isSidebarOpen) => useUiStore.setState({ isSidebarOpen })}
		>
			<TitlebarNav {...props} />
		</SidebarProvider>,
	);
...
describe("TitlebarNav", () => {
	...
	it("uses the compact sidebar-aligned chrome row in native fullscreen", () => {
		const { container } = renderTitlebarNav({ isFullScreen: true });
	...
	it("centers collapsed fullscreen navigation on the inset session topbar", () => {
		...
		const { container } = renderTitlebarNav({ hasSessionTopbar: true, isFullScreen: true });
	...
	it("retains the traffic-light geometry in a windowed macOS shell", () => {
		const { container } = renderTitlebarNav();
