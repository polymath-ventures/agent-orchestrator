...
export function CenterPane({
	...
	const selectAdjacentTab = useCallback(
		(direction: -1 | 1) => {
			const activeIndex =
				target.kind === "shell" ? shellTerminals.findIndex((shell) => shell.handleId === target.handleId) + 1 : 0;
	...
	useEffect(() => {
		aoBridge.app.setCloseShellTerminalShortcutEnabled(target.kind === "shell" && Boolean(onCloseShellTerminal));
