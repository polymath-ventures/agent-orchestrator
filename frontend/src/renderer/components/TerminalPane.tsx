...
type TerminalPaneProps = {
	...
	fontSize: number;
	/** The owner activated this terminal surface and wants keyboard focus. */
	autoFocus?: boolean;
	/** Bump when an already-mounted or retained terminal should take focus. */
	focusRequest?: number;
	/** Move keyboard focus back to the active terminal tab. */
	onExitFocus?: () => void;
...
function terminalTargetMatches(left?: TerminalTarget, right?: TerminalTarget): boolean {
	...
	if (left.kind === "shell" && right.kind === "shell") {
		return left.handleId === right.handleId && left.generation === right.generation && left.title === right.title;
	}
	return false;
}
...
function terminalPropsMatch(left: TerminalPaneProps, right: TerminalPaneProps): boolean {
	return (
		...
		left.focusRequest === right.focusRequest &&
		left.inputDisabled === right.inputDisabled &&
		left.onExitFocus === right.onExitFocus &&
...
function setTerminalPhase(entry: CachedTerminalEntry, phase: CachedTerminalEntry["activationPhase"]): void {
...
function CachedTerminalPortal({
	...
	const handleFatal = useCallback((message: string) => onFatal(entry.cacheKey, message), [entry.cacheKey, onFatal]);
	...
	}, [active, entry, entry.activationId, entry.activationPhase, entry.terminal, onPrepared]);
	useLayoutEffect(() => {
...
export function TerminalCacheProvider({
	...
	if (!muxPoolRef.current) {
		muxPoolRef.current = createTerminalMuxPool(() => createTerminalMux(muxUrlFromApiBase(getApiBaseUrl())));
	}
	...
	useEffect(() => {
		...
		for (const entry of entriesRef.current.values()) {
			...
			if (entry.kind === "worker" && session && session.terminalHandleId !== entry.handleId) {
				removeEntry(entry.cacheKey);
				continue;
			}
	...
	useEffect(() => {
		...
		const shells = new Map((shellTerminalsQuery.data ?? []).map((terminal) => [terminal.handleId, terminal] as const));
		for (const entry of entriesRef.current.values()) {
			if (entry.kind !== "shell") continue;
			const shell = shells.get(entry.handleId);
			if (!shell || shell.createdAt !== entry.generation || shell.sessionId !== entry.sessionId) {
				removeEntry(entry.cacheKey);
				continue;
			}
...
function CachedTerminalSlot({ descriptor, props }: { descriptor: TerminalCacheDescriptor; props: TerminalPaneProps }) {
...
export function TerminalPane({
	...
	const terminalTarget =
		requestedTerminalTarget && terminalTargetBelongsToSession(requestedTerminalTarget, session?.id)
...
function AttachedTerminal({
			...
			if (!urlWatcherRef.current) {
				urlWatcherRef.current = createUrlWatcher(() => {
					...
					const viewingBrowser =
						isVisibleRef.current && (current?.isOpen ?? true) && (current?.view ?? "summary") === "browser";
	...
	const showReplayCover =
		Boolean(handleId) && (!replaySettled || replayPaintPending) && (state === "connecting" || state === "attached");
