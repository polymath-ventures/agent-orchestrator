	...
	it("shows that failing CI was not injected while preserving the failing checks", () => {
		const failingPR = prSummary(7, "open", {
			ci: {
				autoInjectCI: false,
				state: "failing",
				failingChecks: [{ name: "unit", status: "failed", conclusion: "failure", url: "https://ci.example/unit" }],
			},
...
describe("SessionInspector Usage & cost section", () => {
	...
	it("hides the section when an observed usage record has only zero token values", async () => {
		...
		const { queryClient } = renderWithQuery(<SessionInspector session={session([])} />);
		await waitFor(() => expect(queryClient.getQueryData(sessionUsageDetailQueryKey("sess-1"))).toEqual(zeroUsage));
...
describe("SessionInspector summary reviews", () => {
	...
	it("shows eligible and up-to-date open PR review rows", async () => {
		...
		expect(
			within(
				screen.getByText("Reviewable change 4").closest("[data-testid='review-pr-row']") as HTMLElement,
