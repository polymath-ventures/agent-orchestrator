		...
		<div className={cn("group/message relative", visiblyStreaming && hasText && "chat-assistant-streaming")}>
			...
			{visiblyStreaming ? (
				...
				<div className="flex h-[18px] items-center opacity-0 transition-opacity duration-150 focus-within:opacity-100 group-hover/message:opacity-100">
					...
					<CopyButton text={message.text} label="Copy message as markdown" compact className="-ml-1.5" />
				</div>
			) : null}
		</div>
	);
		...
		<>
			...
			{detail?.outputTruncated ? (
				<p className="text-[10px] leading-relaxed text-warning">
					This command printed more than AO stores, so the output above stops early. Open a shell in the worktree to see
					the rest.
				</p>
...
function ActivityState({
	...
	if (status === "running") {
		return (
			<Loader2 aria-label="running" className="size-3 shrink-0 animate-spin self-center text-muted-foreground/60" />
		);
	}
	if (files.length) {
	...
	if (status === "cancelled") {
		return <span className="shrink-0 font-mono text-[10px] text-muted-foreground/70">stopped</span>;
	}
		...
		<div className="chat-code mb-1 mt-0.5 overflow-hidden rounded-md border border-border bg-background">
			...
			{truncated ? (
				<p className="border-t border-border px-2.5 py-1.5 text-[10px] leading-relaxed text-warning">
					This patch is longer than AO stores, so it stops early. The whole change is in the worktree and in the
					turn&rsquo;s diff.
				</p>
			) : null}
		</div>
		...
		<div className="flex gap-2.5 border-l-2 border-border-strong py-0.5 pl-3">
			...
			<div className="min-w-0 flex-1">
				<div className="flex items-center gap-2">
					...
					{streaming ? (
						<Loader2 aria-label="still thinking" className="size-3 animate-spin text-muted-foreground/50" />
					) : null}
				</div>
		...
		<div className="group/activity border-t border-border first:border-t-0">
			...
			>
				...
				{activity.status === "running" ? (
					<Loader2 aria-label="running" className="size-3 shrink-0 animate-spin text-muted-foreground/60" />
				) : failed ? (
			...
			{open && hasBody ? (
				<div className="flex flex-col gap-2 px-[11px] pb-2.5">
					...
					{detail?.progress ? (
						<div className="flex flex-col gap-1">
							<span className="text-[10px] uppercase tracking-[0.08em] text-muted-foreground/70">Progress</span>
		...
		<div className="group/activity border-t border-border first:border-t-0">
			...
			>
				<Icon
					...
					size={13}
				/>
				<strong className="shrink-0 font-medium text-foreground">{denied ? "Auto-declined" : "Auto-approved"}</strong>
				<span
		...
		<div className="flex items-start gap-2.5 rounded-md border border-border bg-surface/60 px-3 py-2">
			...
			<div className="flex min-w-0 flex-col gap-0.5">
				<span className="text-[11px] text-foreground">
					Answered by <strong className="font-medium">{detail?.toModel ?? "another model"}</strong>
					{detail?.fromModel ? (
						<>
							{" "}
...
function ReauthRow({ activity }: { activity: ConversationActivity }) {
	return (
		<div className="flex items-start gap-2.5 rounded-md border border-destructive/40 bg-surface px-3 py-2">
			...
			<div className="flex min-w-0 flex-col gap-0.5">
				<strong className="text-[11px] font-medium text-destructive">The provider asked you to sign in again</strong>
	...
	return (
		...
		>
			<div className="flex items-center gap-2 border-b border-border px-3.5 py-2.5">
				...
				<span className="ml-auto shrink-0 font-mono text-[11px] text-muted-foreground">req {activity.requestId}</span>
			</div>
			...
			<div className="flex flex-col gap-2.5 px-3.5 py-3">
				...
				{resolved ? (
					<p className="text-[11px] text-muted-foreground">Already answered. This card is kept for the record.</p>
				) : (
	...
	return (
		<div className="rounded-lg border border-border bg-surface">
			...
			>
				...
				{live ? (
					<Loader2 aria-label="still changing" className="size-3 shrink-0 animate-spin text-muted-foreground/60" />
				) : null}
				<span className="flex-1" />
...
export function TurnOutcome({
	...
	return (
		<div className="group/turn flex items-center gap-2 pt-1">
			...
			<span className={cn("shrink-0 text-[10px] uppercase tracking-[0.08em]", copy.tone)}>{copy.label}</span>
