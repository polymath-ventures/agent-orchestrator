...
describe("ReauthBanner", () => {
	...
	it("says the worktree is untouched, since nothing else about the session works", () => {
		render(<ReauthBanner account={{ reauthRequiredAt: "2026-08-03T00:00:00Z" }} harness="codex" />);
	...
	it("names Claude Code's non-interactive authentication command", () => {
		render(<ReauthBanner account={{ reauthRequiredAt: "2026-08-03T00:00:00Z" }} harness="claude-code" />);
	...
	it("falls back to generic wording rather than guessing a command", () => {
		render(<ReauthBanner account={{ reauthRequiredAt: "2026-08-03T00:00:00Z" }} harness="opencode" />);
	...
	it("stays silent for an account with no credential demand", () => {
		const { container } = render(<ReauthBanner account={{ authMode: "chatgpt", planLabel: "Pro" }} harness="codex" />);
...
describe("ThreadStateBanner", () => {
	...
	it("lists what the provider says it is waiting on", () => {
		render(<ThreadStateBanner threadState={{ status: "system_error", waitingOn: ["user_input"] }} />);
...
describe("McpServerBanner", () => {
	...
	it("disables the reload mid-turn and says why", () => {
		...
		expect(button).toHaveAttribute("title", expect.stringContaining("Finish or stop the current turn"));
	});
