...
import { applySuggestion, findActiveTrigger, moveHighlight, rankFiles, rankSkills } from "./composerSuggest";
...
describe("rankFiles", () => {
	...
	it("scores the basename ahead of the full path", () => {
		...
		expect(ranked[0]?.value).toBe("frontend/src/renderer/components/chat/ChatComposer.tsx");
	});

	it("narrows on a directory fragment", () => {
		expect(rankFiles(paths, "ports/").map((item) => item.value)).toEqual(["backend/internal/ports/chat.go"]);
	});
...
describe("applySuggestion", () => {
	...
	it("quotes a path containing whitespace", () => {
		...
		expect(applySuggestion("@my", trigger, "my notes/todo.md").text).toBe('"my notes/todo.md" ');
	});
});
