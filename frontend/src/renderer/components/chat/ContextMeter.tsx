	...
	if (!contextWindow || contextWindow <= 0) {
		return (
			<Tooltip>
				...
				<TooltipContent>
					<p>This model does not report a context window, so how full the conversation is is unknown.</p>
					{usage.cost != null ? (
						<p className="mt-1 tabular-nums">Provider-reported cost: {formatCost(usage.cost, usage.currency)}</p>
					) : null}
				</TooltipContent>
			</Tooltip>
		);
		...
		<Tooltip>
			...
			<TooltipContent>
				<p className="tabular-nums">
					{contextUsed.toLocaleString()} of {contextWindow.toLocaleString()} tokens of context used
				</p>
				{severity !== "normal" ? (
					<p className="mt-1">
						{severity === "critical"
