	...
	return (
		<div role="group" aria-label="Turn settings" className="flex min-w-0 flex-col gap-0.5">
			<div className="flex flex-wrap items-center gap-0.5">
				{onChange && models.length > 0 ? (
					...
					>
						<DropdownMenuLabel className="flex items-baseline justify-between gap-2">
							<span>Model</span>
							<span className="text-[11px] font-normal text-muted-foreground">Applies to the next turn</span>
						</DropdownMenuLabel>
				...
				{onChange ? (
					<Picker label={approvalLabel} title="What the agent may do without asking" disabled={disabled} width="w-72">
						<DropdownMenuLabel className="flex items-baseline justify-between gap-2">
							<span>Approvals</span>
							<span className="text-[11px] font-normal text-muted-foreground">Applies to the next turn</span>
						</DropdownMenuLabel>
						{APPROVAL_ORDER.map((mode) => (
							...
							>
								...
								<span className="text-[11px] leading-snug text-muted-foreground">{APPROVAL_COPY[mode].hint}</span>
							</DropdownMenuItem>
						))}
					</Picker>
				) : null}
