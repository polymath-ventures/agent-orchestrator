	...
	return (
		...
		>
			...
			<div className="flex min-w-0 flex-col gap-1">
				<strong className="text-xs font-semibold text-destructive">Sign in again to keep going</strong>
				<p className="text-[11px] leading-relaxed text-foreground">
					{account.reauthReason ?? "The provider rejected this session's credentials."} Nothing will run until it is
				...
				<p className="text-[11px] leading-relaxed text-muted-foreground">
					{command ? (
						...
						<>
							Sign in with the agent&rsquo;s own CLI, then send your message again. AO holds no credentials of its own.
						</>
					)}
				</p>
			</div>
		...
		>
			...
			<div className="flex min-w-0 flex-1 flex-col gap-1">
				<strong className="text-xs font-medium text-warning">
					{servers.length === 1 ? "A tool server did not start" : `${servers.length} tool servers did not start`}
				</strong>
				<span className="text-[11px] leading-snug text-muted-foreground">
