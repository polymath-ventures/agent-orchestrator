				...
				? {
						...
						plan: {
							...
							steps: turn.plan.steps.map((step) =>
								step.text === "Run the backend tests" ? { ...step, status: "completed" as const } : step,
							),
						},
					}
				: turn,
...
describe("model reroute", () => {
	it("names what answered rather than what was asked for", () => {
		render(<ChatWorkspace snapshot={chatFixtureRerouted} models={MODELS} onChooseSettings={vi.fn()} />);
	...
	it("carries the provider's own reason", () => {
		render(<ChatWorkspace snapshot={chatFixtureRerouted} models={MODELS} onChooseSettings={vi.fn()} />);
		expect(screen.getByText(/The requested model is at capacity for this account tier/)).toBeInTheDocument();
	});
