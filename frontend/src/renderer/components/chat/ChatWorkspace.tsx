				...
				description={
					<>
						...
						<p className="mt-1 text-xs text-muted-foreground">
							Its memory of the conversation is discarded up to this point, so it will not know about anything you or it
							said later. Files it already changed in the worktree are left exactly as they are; only the conversation
							is rolled back. This cannot be undone.
						</p>
					</>
				}
...
function Timeline({
	...
	const editedMessageVisible = Boolean(
		messageEdit &&
		items.some((item) => item.kind === "message" && item.role === "user" && item.turnId === messageEdit.turnId),
	);
		...
		<div className="relative min-h-0 flex-1">
			...
			>
				<div ref={scrollContent} className="mx-auto flex max-w-3xl flex-col gap-4.5">
					...
					{groups.map((group) => (
						<div key={group.key} data-chat-scroll-anchor={groupHasHumanPrompt(group) ? "" : undefined}>
							<TurnGroup
								group={group}
		...
		<div className="flex min-h-0 flex-1 items-center justify-center px-6">
			<div className="flex max-w-sm flex-col items-center gap-2 text-center">
				...
				<p className="text-xs leading-relaxed text-muted-foreground">
					This session talks to {harness} over a structured connection. Tool calls, file changes, and approvals appear
					here as they happen. The Terminal tab opens a plain shell in the same worktree — it is not a second copy of
					the agent.
				</p>
			</div>
		</div>
