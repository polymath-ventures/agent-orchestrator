...
describe("TurnChangedFiles", () => {
	...
	it("shows both ends of a rename, so it does not read as an addition", async () => {
		render(
			<TurnChangedFiles
				diff={{
					files: [{ path: "src/new.ts", oldPath: "src/old.ts", additions: 1, deletions: 1, status: "renamed" }],
				}}
			/>,
		);
		await userEvent.click(screen.getByRole("button"));
...
describe("ActivityRow command output", () => {
	...
	it("explains a streamed partial as a dropped beginning, not a generic caveat", () => {
		render(
			<ActivityRow
				activity={commandActivity({ output: "tick-2\n", outputSource: "stream", outputMayBePartial: true }, "running")}
			/>,
		);
	...
	it("keeps a finished command collapsed so the timeline stays readable", () => {
		render(<ActivityRow activity={commandActivity({ output: "ok\n", outputSource: "aggregate" }, "completed")} />);
...
describe("ActivityRun with a streaming command", () => {
	...
	it("opens itself so live output inside it is visible", () => {
		const { container } = render(
			<ActivityRun
				activities={[
					commandActivity({ output: "compiling…\n", outputSource: "stream", outputMayBePartial: true }, "running"),
					plan("act-2"),
				]}
			/>,
		);
	...
	it("stays collapsed when nothing inside it is printing", () => {
		const { container } = render(
			<ActivityRun
				activities={[commandActivity({ output: "done\n", outputSource: "aggregate" }, "completed"), plan("act-2")]}
			/>,
		);
...
describe("ActivityRow command labels", () => {
	...
	it("describes execution as a command", () => {
		render(<ActivityRow activity={commandActivity({ command: "go test ./...", output: "ok" }, "completed")} />);
	...
	it("uses the same compact treatment as a grouped command summary", () => {
		...
		expect(screen.getByText("Checked repository")).toHaveClass("text-[11.5px]", "font-normal", "text-muted-foreground");
	...
	it("shows an interrupted command as stopped instead of leaving a live spinner", () => {
		render(<ActivityRow activity={commandActivity({ command: "sleep 60", output: "started\n" }, "cancelled")} />);
