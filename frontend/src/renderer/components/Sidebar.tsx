...
type SidebarProps = {
	...
	/** Prime remains visible while enabled even when its runtime is down. */
...
function SessionStatusDot({ session }: { session: WorkspaceSession }) {
	...
	const dotClass = activity.state === "active" ? "bg-status-working animate-status-pulse" : activity.indicatorClassName;
	return <span aria-hidden="true" className={cn("size-2 shrink-0 rounded-full", dotClass)} data-session-status="" />;
}
...
export function Sidebar({
	...
	return (
		...
		>
			...
			{/* Keep Prime + Search + section chrome fixed; only the project tree scrolls. */}
...
function PrimeItem({ session, active, onOpen }: { session: WorkspaceSession; active: boolean; onOpen: () => void }) {
	const harnessLabelId = `sidebar-harness-${session.id}`;
	return (
		<SidebarMenuItem>
			<SidebarMenuButton
				aria-current={active ? "page" : undefined}
				aria-describedby={harnessLabelId}
			...
			>
				...
				<span id={harnessLabelId} className="sr-only">
					{session.provider}
				</span>
			</SidebarMenuButton>
		</SidebarMenuItem>
...
function ProjectItem({
	...
	const orchestratorActive =
		...
		workspace.sessions.some((session) => session.id === selection.activeSessionId && session.kind === "orchestrator");
					...
					>
						...
						<div className="transition-[transform] duration-[100ms] ease-out active:scale-[0.98]">
							...
							>
								...
								<span
									aria-hidden="true"
									className="relative shrink-0 group-data-[collapsible=icon]:hidden inline-flex size-[18px] translate-y-px items-center justify-center text-muted-foreground"
								...
								>
									{rowHovered ? (
										<motion.span
											...
											initial={false}
											transition={
												prefersReducedMotion ? { duration: 0 } : { duration: 0.14, ease: [0.25, 0.46, 0.45, 0.94] }
											}
						...
						>
							...
							<DropdownMenu>
								<DropdownMenuTrigger asChild>
									<button
										aria-label={t("shell.projectActions", { name: workspace.name })}
	...
	return (
		<DropdownMenu>
			<DropdownMenuTrigger asChild>
				...
				>
					<User aria-hidden="true" />
					<span className="min-w-0 flex-1 truncate tracking-tight">{session?.user.email ?? "AO Cloud"}</span>
				</button>
			</DropdownMenuTrigger>
