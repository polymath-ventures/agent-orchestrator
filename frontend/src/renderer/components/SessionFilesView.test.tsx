...
describe("SessionFilesView", () => {
	...
	it("shows a minimize action while maximized", async () => {
		const onToggleMaximized = vi.fn();
		renderWithQuery(<SessionFilesView isMaximized onToggleMaximized={onToggleMaximized} sessionId="sess-1" />);
				...
				if (path === "/api/v1/sessions/{sessionId}/workspace/files") {
					return {
						data: {
							...
							compareMode: "base",
							files: [
								{ path: "src/App.tsx", status: "modified", additions: 1, deletions: 2, size: 120, binary: false },
							],
						},
					};
				}
