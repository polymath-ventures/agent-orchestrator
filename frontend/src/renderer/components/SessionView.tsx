...
import { interfaceTransitionIsActive, useSessionInterfaceTransition } from "../hooks/useSessionInterfaceTransition";
...
export function SessionView({ sessionId }: SessionViewProps) {
	...
	const selectReviewerTerminal = useCallback(
		(terminal: ReviewerTerminalTarget) => {
			setActiveShellTerminal(null);
			setTerminalTarget({
				kind: "reviewer",
		...
		[requestTerminalFocus, sessionId, setActiveShellTerminal],
	);
	...
	}, [activeShellTerminalHandleId, requestTerminalFocus, sessionId, shellTerminals]);
	...
	const chatControllerTransitioning = Boolean(
		interfaceSwitch.transition?.targetMode === "chat" && (activeInterfaceTransition || interfaceSwitch.settling),
	);
		...
		<div className="relative flex h-full min-h-0 flex-col bg-background text-foreground" data-testid="session-detail">
				...
				<ResizablePanel defaultSize="72%" id="terminal" minSize="45%" style={{ overflow: "hidden" }}>
					<div className="relative h-full min-h-0">
						...
						{showChatSurface ? (
							...
							<CenterPane
								agentInputDisabled={(interfaceSwitch.starting || activeInterfaceTransition) && session?.mode === "tui"}
								daemonReady={daemonStatus.state === "ready"}
								focusRequest={browserPoppedOut || filesPoppedOut ? undefined : terminalFocusRequest}
			...
			{filesPoppedOut && session
				? createPortal(
						...
						>
							<SessionFilesView isMaximized onToggleMaximized={handleToggleFilesPopOut} sessionId={session.id} />
						</div>,
						document.body,
					)
				: null}
