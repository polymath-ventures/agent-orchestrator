			...
			<DialogContent className="max-w-md gap-0 overflow-hidden rounded-xl border-border bg-popover p-0">
				<DialogHeader className="border-b border-border px-5 py-4">
					...
					<DialogDescription className="pt-1 text-xs leading-5">
						The same AO session, worktree, and agent-native conversation continue in the other interface. Completed
						messages and tool work stay in the agent's context.
					</DialogDescription>
				</DialogHeader>
				...
				<div className="grid gap-2 px-5 py-4">
					...
					>
						...
						<span className="mt-1 block text-xs leading-5 text-muted-foreground">
							Cancel the running turn before switching. Files already changed remain in the worktree, but unfinished
							output and queued Chat turns are cancelled.
						</span>
					</button>
					{waitingForInput ? (
						<p className="text-[11px] leading-4 text-warning">
							This turn is waiting for your input. “Finish work” will wait until you answer it; use “Stop now” to switch
							immediately.
						</p>
					) : null}
					{error ? (
