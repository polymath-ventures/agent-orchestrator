...
async function fetchDevices(): Promise<MobileDevice[]> {
	...
	if (error) throw new MobileDevicesQueryError(apiErrorMessage(error), apiErrorCode(error));
	if (!data) return [];
	return Array.isArray(data.devices) ? (data.devices as MobileDevice[]) : [];
}
							...
							return (
								...
								>
									...
									<Switch
										...
										onCheckedChange={(next) => mute.mutate({ installId: device.installId, muted: !next })}
										className={cn(
