		...
		>
			<DropdownMenuTrigger asChild>
				<button
					...
					aria-label={ariaLabel}
				>
					{renderTrigger ? renderTrigger(currentLabel) : <span className="min-w-0 truncate">{currentLabel}</span>}
...
function readRecentModelMap(): Record<string, string[]> {
	...
	try {
		...
		for (const [scope, values] of Object.entries(parsed)) {
			...
			result[scope] = values.filter((value): value is string => typeof value === "string").slice(0, MAX_RECENT_MODELS);
		}
		return result;
	} catch {
		return {};
...
function rememberRecentModel(scope: string, modelID: string): string[] {
	...
	const next = [modelID, ...(recentModels[scope] ?? []).filter((id) => id !== modelID)].slice(0, MAX_RECENT_MODELS);
...
export function buildModelSearchIndex(models: AgentModel[]): ModelSearchIndex {
	...
	for (const item of indexedModels) {
		const providerKeys = new Set([item.normalizedProvider, normalizeSearch(providerFromModelID(item.id))]);
