...
describe("KeyboardShortcutsSettingsDialog", () => {
	...
	it("adds, removes, resets, and resets all bindings", async () => {
		...
		await waitFor(() => expect(Object.hasOwn(useKeybindingsStore.getState().overrides, "focus-terminal")).toBe(false));
	...
	it("uses the platform prop for displayed and recorded bindings", async () => {
		...
		await waitFor(() => expect(useKeybindingsStore.getState().overrides["focus-terminal"]?.[0]?.meta).toBe(true));
