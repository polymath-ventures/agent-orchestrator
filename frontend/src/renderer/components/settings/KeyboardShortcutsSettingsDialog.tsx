	...
	const filteredShortcuts = useMemo(() => {
		...
		return APP_SHORTCUTS.filter((shortcut) => {
			const labels = effectiveShortcutBindings(shortcut.id, isMac, overrides)
				...
				.join(" ");
			return `${shortcutLabel(shortcut.id, t)} ${shortcutCategoryLabel(shortcut.category, t)} ${labels}`
				.toLowerCase()
				.includes(needle);
		});
			...
			if (conflicting) {
				if (conflicting.customizable === false) {
					showToast({
						...
						body: t("shortcut.usedByFixed", {
							binding: shortcutBindingLabel(candidate, isMac),
							name: shortcutLabel(conflicting.id, t),
						}),
					});
					return;
				}
										...
										<div className="flex items-center gap-3">
											<div className="min-w-0 flex-1">
												...
												<span className="text-caption text-settings-muted">
													{shortcutCategoryLabel(shortcut.category, t)}
												</span>
											</div>
																...
																<Tooltip>
																	<TooltipTrigger asChild>
																		<button
																			...
																			aria-label={t("shortcut.removeBindingAria", {
																				binding: shortcutBindingLabel(candidate, isMac),
																				name: shortcutLabel(shortcut.id, t),
																			})}
