...
const interfaceTransitionState = vi.hoisted(() => ({
	status: undefined as { supported: boolean; targetMode?: "chat" | "tui"; reason?: string } | undefined,
}));
...
vi.mock("./CenterPane", () => ({
	CenterPane: ({
		...
		<div>
			terminal center
			{topbarActions}
			<div data-testid="terminal-target">{terminalTarget?.kind === "shell" ? terminalTarget.handleId : "worker"}</div>
...
describe("SessionView", () => {
	...
	it.each([
		...
		(_label, sessionId, mode, targetMode, buttonName, status, activityState) => {
			interfaceTransitionState.status = { supported: true, targetMode };
			const session = workerSession(sessionId);
			session.mode = mode;
