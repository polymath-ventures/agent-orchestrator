...
export type AttachableTerminal = {
	cols: number;
	rows: number;
	/** Focus xterm's hidden input once its retained container is interactive. */
	focus: () => void;
	/**
...
export type UseTerminalSessionOptions = {
	...
	/**
	 * Observe decoded pane output. Callers use it to scan the stream for signals
	 * like printed URLs; it must be cheap because it runs on every output chunk.
	 */
...
export function useTerminalSession(session: WorkspaceSession | undefined, options: UseTerminalSessionOptions) {
	...
	const connect = useCallback(() => {
		...
		r.flushReplay = (preserveBeforeTeardown = false) => flushReplay(false, preserveBeforeTeardown);
		...
		r.lastPublishedGrid = openCols > 0 && openRows > 0 ? { cols: openCols, rows: openRows } : null;
