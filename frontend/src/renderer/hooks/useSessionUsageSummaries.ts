...
export const sessionUsageQueryKey = (projectId?: string) => [...sessionUsageQueryRoot, projectId ?? "all"] as const;
...
export function sessionUsageQueryOptions(projectId?: string) {
	return {
		...
		retry: 1,
		select: (items: SessionUsageSummary[]) => new Map(items.map((item) => [item.sessionId, item] as const)),
	};
}
