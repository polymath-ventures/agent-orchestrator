...
export type SessionInterfaceTransitionStatus = components["schemas"]["SessionInterfaceTransitionStatusResponse"];
...
const cancellablePhases = new Set<SessionInterfaceTransition["phase"]>(["requested", "preflighting", "draining"]);
...
export function useSessionInterfaceTransition(sessionId: string | undefined) {
	...
	const query = useQuery({
		...
		queryFn: async () => {
			const { data, error } = await apiClient.GET("/api/v1/sessions/{sessionId}/interface-transition", {
	...
	const start = useMutation({
		mutationFn: async (input: { targetMode: SessionInterfaceMode; policy: SessionInterfaceTransitionPolicy }) => {
			const { data, error } = await apiClient.POST("/api/v1/sessions/{sessionId}/interface-transition", {
				params: { path: { sessionId: sessionId as string } },
				body: input,
			});
			if (error) throw error;
	...
	const cancel = useMutation({
		mutationFn: async () => {
			const { error } = await apiClient.DELETE("/api/v1/sessions/{sessionId}/interface-transition", {
				params: { path: { sessionId: sessionId as string } },
			});
			if (error) throw error;
		},
	...
	useEffect(() => {
		...
		void Promise.all([
			...
			if (!current) return;
			setRefreshingTransitionID((refreshing) => (refreshing === transitionID ? "" : refreshing));
		});
		return () => {
			current = false;
		};
