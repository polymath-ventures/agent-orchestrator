...
const CLIENT_ID = import.meta.env.VITE_WORKOS_CLIENT_ID?.trim() || (process.env.VITEST ? "client_test" : "");
...
function toStoredSession(accessToken: string, refreshToken: string, user: User): StoredSession {
	return {
		accessToken,
		refreshToken,
		authProvider: "workos",
...
async function refreshCloudSession(
	...
	try {
		...
		const session = toStoredSession(refreshed.accessToken, refreshed.refreshToken, refreshed.user);
...
export async function handleCloudDeepLink(rawURL: string, dataDir: string): Promise<CloudAccount | null> {
	if (!workos) throw new Error("WorkOS is not configured.");
	const url = new URL(rawURL);
	if (url.protocol !== "ao-app:" || url.hostname !== "callback") return null;
	const error = url.searchParams.get("error");
	if (error) {
		throw new Error(url.searchParams.get("error_description") || `WorkOS sign-in failed: ${error}`);
	}
	...
	const session = toStoredSession(result.accessToken, result.refreshToken, result.user);
...
function signInFailureDetail(error: unknown): string {
	...
	if (message.includes("state") || message.includes("pending") || message.includes("incomplete")) {
	...
	ipcMain.handle("cloud:signIn", async () => {
		if (!workos) {
			await dialog.showMessageBox({
				...
				detail: "Set VITE_WORKOS_CLIENT_ID and restart Agent Orchestrator to enable sign-in.",
			});
			return;
		}
