...
import { decideRelocation, installedBundlePath, readBundleVersion, type RelocationInputs } from "./relocation";
...
describe("decideRelocation", () => {
	...
	it("hands off when the install is the same version", () => {
		expect(decideRelocation({ ...BASE, installedVersion: "0.10.3" })).toBe("handoff");
	});
	...
	it("stays put when already in an Applications folder", () => {
		expect(decideRelocation({ ...BASE, inApplicationsFolder: true })).toBe("stay");
	});
	...
	it("stays put when either version is not semver", () => {
		expect(decideRelocation({ ...BASE, installedVersion: "garbage" })).toBe("stay");
		expect(decideRelocation({ ...BASE, runningVersion: "garbage" })).toBe("stay");
	});
});

describe("installedBundlePath", () => {
	it("maps any running bundle to the same name under /Applications", () => {
		expect(installedBundlePath("/Users/x/Downloads/Agent Orchestrator.app")).toBe(
			"/Applications/Agent Orchestrator.app",
		);
		expect(installedBundlePath("/Volumes/Agent Orchestrator/Agent Orchestrator.app")).toBe(
			"/Applications/Agent Orchestrator.app",
		);
	});
});
