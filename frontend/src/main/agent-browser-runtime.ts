...
export class AgentBrowserRuntime {
	...
	async run(
		...
		const disabled = await this.processRunner(this.options.binaryPath, ["stream", "disable"], environment, signal);
	...
	private async ensureSession(sessionId: string, provider: AgentBrowserTargetProvider): Promise<SessionRuntime> {
		if (this.disposed) throw runtimeError("AGENT_BROWSER_RUNTIME_CLOSED", "Browser automation runtime is closed");
...
function streamAlreadyDisabled(result: NativeProcessResult): boolean {
	return (
		result.stderr.includes(STREAM_ALREADY_DISABLED_MESSAGE) || result.stdout.includes(STREAM_ALREADY_DISABLED_MESSAGE)
	);
}
...
export function parseAgentBrowserJSON(stdout: string): AgentBrowserJSONResult {
	...
	if (!isRecord(envelope))
		throw runtimeError("AGENT_BROWSER_INVALID_OUTPUT", "Browser automation returned invalid output");
