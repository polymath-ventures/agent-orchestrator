...
const ACCESS_TOKEN = `header.${Buffer.from(JSON.stringify({ exp: 4_102_444_800, sid: "session_123" })).toString(
	"base64url",
)}.signature`;
const EXPIRED_ACCESS_TOKEN = `header.${Buffer.from(JSON.stringify({ exp: 1, sid: "session_123" })).toString(
	"base64url",
)}.signature`;
...
describe("native WorkOS authentication", () => {
	...
	it("starts PKCE and exchanges the callback without an AO website", async () => {
		...
		expect(mocks.openExternal).toHaveBeenCalledWith("https://workos.example/authorize");
		...
		const session = await handleCloudDeepLink("ao-app://callback?code=code_123&state=state_123", dataDir);
	...
	it("rejects callbacks whose OAuth state does not match", async () => {
		await beginCloudSignIn(dataDir);
		await expect(handleCloudDeepLink("ao-app://callback?code=code_123&state=attacker_state", dataDir)).rejects.toThrow(
			"state did not match",
		);
	});
	...
	it("shows a bounded error when the callback cannot be completed", async () => {
		await showCloudSignInFailure(new Error("The WorkOS sign-in request expired with secret details"));

		expect(mocks.showMessageBox).toHaveBeenCalledWith({
			type: "error",
			title: "AO Cloud sign-in failed",
			message: "Unable to sign in to AO Cloud",
			detail: "The WorkOS sign-in request expired. Start sign-in again to continue.",
		});
	});
	...
	it("shares one rotating-token refresh between concurrent callers", async () => {
		...
		await vi.waitFor(() => expect(mocks.authenticateWithRefreshToken).toHaveBeenCalledOnce());
		resolveRefresh?.({
			accessToken: ACCESS_TOKEN,
	...
	it("clears an expired session when token refresh fails", async () => {
		...
		mocks.authenticateWithRefreshToken.mockRejectedValueOnce(new Error("refresh token already consumed"));
		await beginCloudSignIn(dataDir);
		await handleCloudDeepLink("ao-app://callback?code=code_123&state=state_123", dataDir);
