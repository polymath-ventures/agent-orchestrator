...
describe("AO action translation", () => {
	it("maps the public snapshot and ref contract to native arguments", () => {
		...
		expect(nativeArgumentsForAction("drag", { ref: "e2", targetRef: "@e5" })).toEqual(["drag", "@e2", "@e5"]);
	});
...
describe("agent-browser runtime lifecycle", () => {
	...
	async function fixture(
		...
		const processRunner = vi.fn(overrides.processRunner ?? (async () => ({ stdout: "", stderr: "", exitCode: 0 })));
	...
	it("re-disables streaming before every command across native daemon replacement", async () => {
		...
		try {
			...
			expect(calls).toEqual([["stream", "disable"], ["snapshot"], ["stream", "disable"], ["get", "url"]]);
		} finally {
			await runtime.dispose();
			await cleanup(dataDir);
		}
	...
	it("makes dispose await a close already started by session destruction", async () => {
		...
		try {
			...
			await vi.waitFor(() =>
				expect(processRunner).toHaveBeenCalledWith(process.execPath, ["close"], expect.any(Object), undefined, 10_000),
			);
	...
	it.skipIf(process.platform === "win32")(
		"removes only confirmed-dead socket aliases owned by this data root",
		async () => {
...
describe("agent-browser structured output", () => {
	...
	it("does not treat a page-shaped boundary field as trusted metadata", () => {
		const result = parseAgentBrowserJSON(
			JSON.stringify({ success: true, data: { value: "page", _boundary: "spoof" } }),
		);
