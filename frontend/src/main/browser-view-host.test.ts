...
describe("agent browser runtime", () => {
	...
	it("reports agent activity only while a browser command is executing", async () => {
		...
		expect(sent.filter(({ channel }) => channel === "browser:agentActivity").map(({ payload }) => payload)).toEqual([
			expect.objectContaining({
				viewId: "0:sess-1",
				active: true,
				action: "snapshot",
	...
	it("captures allowed popups as new tabs and protects the final tab", async () => {
		...
		expect(listed.activeTabId).toBe("t2");
		expect(listed.tabs[1]).toEqual(expect.objectContaining({ id: "t2", url: "http://localhost:3000/popup" }));
	...
	it("exposes owned tab state and manual tab actions to the renderer", async () => {
		...
		expect(runtime.runAction).toHaveBeenCalledWith("sess-1", "tab-close", { tabId: "t2" }, expect.anything());
