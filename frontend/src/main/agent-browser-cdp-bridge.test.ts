...
import { AgentBrowserCDPBridge, type AgentBrowserDebugger, type AgentBrowserTarget } from "./agent-browser-cdp-bridge";
...
describe("worker-scoped agent-browser CDP bridge", () => {
	...
	it("routes flattened cross-origin iframe sessions only through their owning target connection", async () => {
		...
		try {
			...
			firstDebug.emit("message", {}, "Target.attachedToTarget", {
			...
			firstDebug.emit("message", {}, "Target.detachedFromTarget", {
				sessionId: "chromium-oopif-session",
				targetId: "oopif-1",
			});
			await detachedEvent;
