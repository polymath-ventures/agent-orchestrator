...
export function RoadmapSlideshow({ phases }: { phases: RoadmapPhase[] }) {
						...
						<div key={phase.title} className="border-b border-border">
							...
							>
								<span className="flex items-start gap-4">
									<span className="mt-1 w-6 shrink-0 text-xs font-medium text-muted-foreground">{phase.num}</span>
									<span className="min-w-0 flex-1">
										<span className="flex items-start justify-between gap-4">
											<span className="text-lg font-medium tracking-[-0.5px] text-foreground">{phase.title}</span>
											<span
												aria-hidden="true"
										...
										<span className="mt-1 block text-xs leading-5 text-muted-foreground">{phase.status}</span>
									</span>
								</span>
									...
									{isActive ? (
										...
										>
											<span className="block pl-10 pt-5 lg:h-[17rem] lg:pr-4">
												<span className="block max-w-lg text-sm leading-7 text-muted-foreground">{phase.theme}</span>
												<span className="mt-5 block space-y-3 border-l border-border pl-5">
													{phase.unlocks.map((unlock) => (
														<span key={unlock} className="block text-sm leading-6 text-muted-foreground">
															{unlock}
														</span>
													))}
												</span>
