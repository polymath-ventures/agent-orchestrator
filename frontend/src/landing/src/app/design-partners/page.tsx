...
import { formatStatPlus, getGitHubRepoStats, monthsSince } from "@/lib/github-stats";
import { RoadmapSlideshow, type RoadmapPhase } from "./RoadmapSlideshow";
...
const phases: RoadmapPhase[] = [
	{
		...
		imageAlt: "One orchestration workstation coordinating five coding-agent modules across separate worktree lanes",
	},
	{
		num: "01",
		...
		imageAlt: "Three local agent workstations connecting to a shared team mission-control board",
	},
	{
		num: "02",
		...
		imageAlt: "A measurement console joining agent cost, merged work, cycle time, and human handoffs",
	},
	{
		num: "03",
		...
		imageAlt: "A self-hosted control plane, security gate, and archive contained inside a private perimeter",
	},
];
...
function SectionLabel({ children }: { children: React.ReactNode }) {
	return <p className="text-sm font-medium tracking-[-0.5px] text-muted-foreground">{children}</p>;
}
				...
				<div className="relative mx-auto max-w-7xl">
					<div className="grid items-center gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.82fr)] lg:gap-8">
						<div>
							...
							<p className="mt-5 text-sm text-muted-foreground">paid pilots · month-to-month · cancel anytime</p>
						</div>
					...
					<div className="mt-16 grid overflow-hidden rounded-xl border border-border sm:grid-cols-2 lg:grid-cols-4">
						{stats.map((stat) => (
							...
							>
								<p className="text-xl font-medium tracking-[-0.5px] text-foreground">{stat.value}</p>
				...
				<div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
					<div>
						...
						<p className="mt-6 max-w-xl text-base leading-8 text-muted-foreground">
							Every session shareable. Every outcome measurable. Every transcript captured - and owned by your org. Each
							layer lands in partner workspaces first.
						</p>
					</div>
					<Image
			...
			<section className="border-t border-border px-4 py-16 sm:px-8 sm:py-20 lg:px-[30px]">
				<div className="mx-auto max-w-7xl">
					...
					<p className="mt-8 max-w-4xl text-sm leading-7 text-muted-foreground">
						Best fit: 10-100 engineers. Multiple agent subscriptions in use. Leadership asking what the spend produces.
						AO ships a new build every night - what we promise, you watch arrive.
					</p>
				</div>
			</section>
			...
			<section className="border-t border-border px-4 py-16 sm:px-8 sm:py-20 lg:px-[30px]">
				<div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-8 py-8 lg:flex-row lg:items-center">
					...
					<div className="flex shrink-0 flex-col gap-3 sm:flex-row">
						...
						<TextLink href={MAILTO_HREF}>or write to {CONTACT_EMAIL}</TextLink>
					</div>
				</div>
			</section>
		</main>
