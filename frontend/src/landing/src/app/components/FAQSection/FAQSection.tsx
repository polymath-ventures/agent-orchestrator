...
function FAQAccordionItem({ item, isOpen, onToggle }: { item: FAQItem; isOpen: boolean; onToggle: () => void }) {
	return (
		<div>
			...
			>
				<span className="text-base sm:text-lg font-medium text-foreground pr-4">{item.question}</span>
				<HiPlus
			...
			<AnimatePresence initial={false}>
				{isOpen && (
					...
					>
						<p className="pb-6 text-base text-muted-foreground leading-relaxed pr-12">{item.answer}</p>
					</motion.div>
				)}
			</AnimatePresence>
		</div>
