...
const brief = "Ship GitHub sign-in, cover the callback flow, and update the setup docs.";

export function DelegationDemo() {
	const [stage, setStage] = useState(0);

	useEffect(() => {
		const interval = window.setInterval(() => setStage((current) => (current + 1) % 3), 2100);
			...
			<div className="grid h-[340px] grid-cols-1 sm:h-[318px] sm:grid-cols-[minmax(0,1.08fr)_minmax(176px,0.72fr)]">
				<section className="flex min-w-0 flex-col p-3 sm:border-r sm:border-[var(--preview-border)] sm:p-4">
					<div className="flex items-center gap-2.5">
						...
						<div>
							<div className="text-[12px] font-semibold tracking-[-0.4px]">AO Orchestrator</div>
							<div className="text-[9px] text-[var(--preview-muted-foreground)]">Plans the work. Routes the fleet.</div>
						</div>
					</div>
					...
					<div className="mt-4 rounded-lg border border-[var(--preview-border)] bg-[var(--preview-card)] p-3">
						...
						<p className="mt-2 text-[11px] leading-[17px] text-[var(--preview-card-foreground)]">{brief}</p>
						<button
							type="button"
							onClick={() => setStage(0)}
								...
								{stage === 0 ? (
									...
									<div className="space-y-1">
										...
										<div className="text-[var(--preview-muted-foreground)]/70">
											...
											{stage === 1 ? "creating worktrees…" : "watch checks and route blockers"}
										</div>
									</div>
								)}
							</motion.div>
				...
				<aside className="hidden min-w-0 bg-[var(--preview-background)] p-3 sm:block">
					<div className="flex items-center justify-between">
						<span className="text-[10px] font-semibold text-[var(--preview-muted-foreground)]">Worker queue</span>
						...
						{workers.map((worker, index) => (
							...
							>
								<div className="flex items-center gap-2">
									<img src={worker.icon} alt="" className="size-3.5" draggable="false" />
									<span className="min-w-0 flex-1 truncate text-[10px] font-medium">{worker.task}</span>
									<StatusDot color={stage === 2 ? previewStatus.working : "#6b7280"} pulse={stage === 2} />
								</div>
