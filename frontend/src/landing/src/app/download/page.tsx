...
async function getReleases(): Promise<GitHubRelease[]> {
	try {
		const response = await fetch(`https://api.github.com/repos/${COMPANY.GITHUB_REPO}/releases?per_page=30`, {
...
function assetUrl(release: GitHubRelease | undefined, exactName: string, fallbackPattern?: RegExp) {
	const exact = release?.assets.find((asset) => asset.name === exactName);
	if (exact) return exact.browser_download_url;
	return release?.assets.find((asset) => fallbackPattern?.test(asset.name))?.browser_download_url;
}

function build(label: string, href: string | undefined, channel: DownloadBuild["channel"]): DownloadBuild | null {
...
export default async function DownloadPage() {
	...
	const nightly = releases.find(
		(release) => !release.draft && release.prerelease && release.tag_name.includes("-nightly."),
	);
	...
	const platformDownloads = [
		{
			...
			builds: available([
				...
				build("Mac (Apple silicon)", assetUrl(nightly, "agent-orchestrator-darwin-arm64.zip"), "Nightly"),
				build("Mac (Intel)", assetUrl(nightly, "agent-orchestrator-darwin-x64.zip"), "Nightly"),
			]),
		},
		{
			name: "Windows",
			...
			builds: available([
				...
				build("Windows (x64)", assetUrl(nightly, "agent-orchestrator-win32-x64.exe"), "Nightly"),
			]),
		},
		{
			name: "Linux",
			...
			builds: available([
				...
				build(
					"Linux .deb (x64)",
					assetUrl(stable, "agent-orchestrator-linux-x64.deb", /^agent-orchestrator[_-].*(?:amd64|x86_64)\.deb$/i),
					"Stable",
				),
				build(
					"Linux RPM (x64)",
					assetUrl(stable, "agent-orchestrator-linux-x64.rpm", /^agent-orchestrator-.*x86_64\.rpm$/i),
					"Stable",
				),
				build("Linux AppImage (x64)", assetUrl(nightly, "agent-orchestrator-linux-x64.AppImage"), "Nightly"),
				build("Linux .deb (x64)", assetUrl(nightly, "agent-orchestrator-linux-x64.deb"), "Nightly"),
			]),
		},
	];
					...
					<div className="grid gap-6 md:grid-cols-2">
						<article className="flex h-full flex-col rounded-2xl bg-card p-4 sm:p-5">
							...
							<div className="flex flex-1 flex-col">
								<h2 className="text-xl font-semibold text-foreground">Desktop</h2>
						...
						<article className="flex h-full flex-col rounded-2xl bg-card p-4 sm:p-5">
							<div className="relative mb-5 h-80 overflow-hidden rounded-xl sm:h-[360px]">
								<Image src="/feature.png" alt="" fill sizes="(max-width: 767px) 100vw, 50vw" className="object-cover" />
								<div className="absolute inset-0 bg-background/10" />
								<PhoneAppPreview />
							</div>
						...
						{[
							{
								...
								label: "Recommended",
								description: "Best for most people. Stable builds are the right choice for everyday work.",
							},
							{
							...
							<section key={channel.name}>
								...
								<div className="grid grid-cols-1 gap-6 text-foreground md:grid-cols-3">
									{platformDownloads.map((platform) => {
										...
										const builds = platform.builds.filter((item) => item.channel === channel.name);
													...
													{builds.map((downloadBuild) => (
														...
														>
															<span className="flex items-center gap-3">
																<span className="whitespace-nowrap text-sm">{downloadBuild.label}</span>
																<Download
